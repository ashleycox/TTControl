...
.tabs button[aria-selected=true],.mode-tabs button[aria-pressed=true],.speed-tabs button[aria-selected=true],.cal-steps button[aria-selected=true]{border-color:var(--accent);background:color-mix(in srgb,var(--accent) 12%,var(--panel));box-shadow:inset 0 0 0 2px color-mix(in srgb,var(--accent) 70%,transparent);color:color-mix(in srgb,var(--accent) 78%,var(--text))}
...
<button aria-selected="true" data-tab="dashboard"><span class="ico" aria-hidden="true">D</span>Dashboard</button>
<button aria-selected="false" data-tab="control"><span class="ico" aria-hidden="true">C</span>Control</button>
<button aria-selected="false" data-tab="settings"><span class="ico" aria-hidden="true">S</span>Settings</button>
<button aria-selected="false" data-tab="calibrate"><span class="ico" aria-hidden="true">T</span>Calibrate</button>
<button aria-selected="false" data-tab="network"><span class="ico" aria-hidden="true">N</span>Network</button>
<button aria-selected="false" data-tab="presets"><span class="ico" aria-hidden="true">P</span>Presets</button>
<button aria-selected="false" data-tab="bench"><span class="ico" aria-hidden="true">B</span>Bench</button>
<button aria-selected="false" data-tab="diagnostics"><span class="ico" aria-hidden="true">I</span>Diagnostics</button>
<button aria-selected="false" data-tab="errors"><span class="ico" aria-hidden="true">E</span>Errors</button>
</nav>
...
function renderCalStepper(){const names=["Frequency","Phase","Kick","Brake","Amplitude"],stepper=$("calStepper");if(!stepper)return;stepper.innerHTML=names.map((n,i)=>`<button data-cal-step="${i}" aria-selected="${i===activeCalStep}"><span class="ico" aria-hidden="true">${i+1}</span>${n}</button>`).join("");stepper.querySelectorAll("[data-cal-step]").forEach(b=>b.onclick=()=>{activeCalStep=Number(b.dataset.calStep);renderCalStepper();document.querySelectorAll("#calibrationTools .cal-card").forEach((p,i)=>p.classList.toggle("active-cal",i===activeCalStep))})}
