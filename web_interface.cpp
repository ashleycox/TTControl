...
<style>
:root{color-scheme:light dark;--bg:#ecefeb;--panel:#fff;--panel2:#f5f7f4;--text:#17201c;--muted:#59645e;--line:#c6cec9;--accent:#1b665e;--accent-fill:#185e57;--accent-text:#fff;--good:#236c46;--good-text:#fff;--danger:#a52b24;--danger-text:#fff;--warn:#875900;--focus:#d47c00}
@media (prefers-color-scheme:dark){:root{--bg:#111613;--panel:#1a211d;--panel2:#222b25;--text:#f2f5f1;--muted:#b8c2bc;--line:#465249;--accent:#83d8cb;--accent-fill:#17635b;--accent-text:#fff;--good:#24744d;--good-text:#fff;--danger:#a83d35;--danger-text:#fff;--warn:#e0b35c;--focus:#ffd166}}
body[data-theme=light]{color-scheme:light;--bg:#ecefeb;--panel:#fff;--panel2:#f5f7f4;--text:#17201c;--muted:#59645e;--line:#c6cec9;--accent:#1b665e;--accent-fill:#185e57;--accent-text:#fff;--good:#236c46;--good-text:#fff;--danger:#a52b24;--danger-text:#fff;--warn:#875900;--focus:#d47c00}
body[data-theme=dark]{color-scheme:dark;--bg:#111613;--panel:#1a211d;--panel2:#222b25;--text:#f2f5f1;--muted:#b8c2bc;--line:#465249;--accent:#83d8cb;--accent-fill:#17635b;--accent-text:#fff;--good:#24744d;--good-text:#fff;--danger:#a83d35;--danger-text:#fff;--warn:#e0b35c;--focus:#ffd166}
body[data-theme=contrast],body.contrast{color-scheme:dark;--bg:#000;--panel:#000;--panel2:#101010;--text:#fff;--muted:#fff;--line:#fff;--accent:#75ddff;--accent-fill:#fff;--accent-text:#000;--good:#62f49a;--good-text:#000;--danger:#ff7070;--danger-text:#000;--warn:#ffdf5d;--focus:#ffdf5d}
*{box-sizing:border-box}
[hidden]{display:none!important}
html{background:var(--bg)}
body{margin:0;min-height:100vh;background:var(--bg);color:var(--text);font:16px/1.5 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
button,input,select,textarea{font:inherit}
button,input,select,textarea{border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--text)}
button{min-height:44px;padding:.58rem .88rem;cursor:pointer;display:inline-flex;align-items:center;gap:.5rem;justify-content:center;font-weight:700;transition:border-color .12s ease,background-color .12s ease}
button:hover:not(:disabled){border-color:var(--accent);background:var(--panel2)}
button.primary{background:var(--accent-fill);border-color:var(--accent-fill);color:var(--accent-text)}
button.good{background:var(--good);border-color:var(--good);color:var(--good-text)}
button.danger{background:var(--danger);border-color:var(--danger);color:var(--danger-text)}
button.primary:hover:not(:disabled),button.good:hover:not(:disabled),button.danger:hover:not(:disabled){filter:brightness(.92)}
button.ghost{background:transparent}
button:focus-visible,input:focus-visible,select:focus-visible,textarea:focus-visible,a:focus-visible{outline:4px solid var(--focus);outline-offset:2px}
button:disabled{opacity:.55;cursor:not-allowed}
body.large{font-size:19px}
body.large button,body.large input,body.large select,body.large textarea{min-height:54px}
.skip{position:absolute;left:1rem;top:-5rem;background:var(--panel);border:2px solid var(--accent);padding:.7rem;z-index:20}
.skip:focus{top:1rem}
.ico{font-size:.76rem;font-weight:850;min-width:1.5rem;text-align:center;line-height:1.35}
header{background:var(--panel);border-bottom:1px solid var(--line)}
.top{display:flex;gap:1rem;align-items:center;justify-content:space-between;max-width:1180px;margin:0 auto;padding:.8rem 1rem}
.brand{font-weight:850;font-size:1.38rem;letter-spacing:-.02em;display:flex;align-items:center;gap:.55rem}
.brand:before{content:"";width:.82rem;height:.82rem;border:3px solid var(--accent);border-radius:50%}
.net{color:var(--muted);font-size:.95rem}
.header-tools{display:flex;flex-wrap:wrap;gap:.55rem;justify-content:flex-end;align-items:center}
.toolbar-field{display:flex;align-items:center;gap:.4rem;color:var(--muted);font-size:.9rem;font-weight:700}
.toolbar-field select{width:auto;min-height:38px}
.wrap{max-width:1180px;margin:0 auto;padding:1rem 1rem 2rem}
.safety-bar{position:sticky;top:0;z-index:8;background:var(--panel);border-bottom:1px solid var(--line)}
.safety-inner{max-width:1180px;margin:0 auto;padding:.55rem 1rem;display:flex;gap:.75rem;align-items:center;justify-content:space-between}
.safety-state{min-width:0}.safety-state strong{display:block}.safety-state span{display:block;color:var(--muted);font-size:.9rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.status-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:.85rem}
.metric{background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:.85rem;border-top:3px solid var(--line)}
.metric.ok{border-top-color:var(--good)}
.metric.warn{border-top-color:var(--warn)}
.metric.bad{border-top-color:var(--danger)}
.metric span{display:block;color:var(--muted);font-size:.82rem;font-weight:800;text-transform:uppercase}
.metric strong{display:block;font-size:1.55rem;margin-top:.15rem;line-height:1.1}
.controls,.tabs,.mode-tabs,.speed-tabs,.cal-steps{display:flex;flex-wrap:wrap;gap:.55rem;margin:1rem 0}
.controls{background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:.72rem}
.tabs{align-items:center;border-bottom:1px solid var(--line);padding:0 0 .7rem;margin-top:.85rem}
.tabs button,.mode-tabs button,.speed-tabs button,.cal-steps button{background:var(--panel);font-weight:750}
.tabs button[aria-pressed=true],.mode-tabs button[aria-pressed=true],.speed-tabs button[aria-selected=true],.cal-steps button[aria-current=step]{border-color:var(--accent);background:var(--panel2);color:var(--accent)}
.nav-more{position:relative}.nav-more summary{list-style:none;min-height:44px;padding:.58rem .88rem;border:1px solid var(--line);border-radius:6px;background:var(--panel);font-weight:750;cursor:pointer}.nav-more summary::-webkit-details-marker{display:none}.nav-more summary:after{content:" +"}.nav-more[open] summary:after{content:" −"}.nav-menu{position:absolute;right:0;top:calc(100% + .35rem);z-index:9;width:15rem;padding:.45rem;display:grid;gap:.35rem;background:var(--panel);border:1px solid var(--line);border-radius:6px}.nav-menu button{justify-content:flex-start;width:100%}
main section{display:none}
main section.active{display:block}
.panel{background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:1rem;margin:1rem 0}
#dashboardBody.panel{background:transparent;border:0;box-shadow:none;padding:0;margin:1rem 0}
#dashboardBody>h2{margin:.35rem 0 1rem;font-size:1.5rem}
.panel.section-head{border-left:4px solid var(--accent)}
.panel legend{font-size:1.25rem;font-weight:850;padding:0 .25rem}
h2,h3{line-height:1.15}
.settings-grid,.tool-grid,.bench-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1rem}
.settings-grid fieldset{align-self:start}
.settings-shell{display:grid;grid-template-columns:13rem minmax(0,1fr);gap:1rem;align-items:start}.settings-nav{position:sticky;top:4.5rem;display:grid;gap:.35rem}.settings-nav a{color:var(--text);text-decoration:none;padding:.5rem .6rem;border-left:3px solid transparent}.settings-nav a:hover,.settings-nav a:focus-visible{background:var(--panel2);border-left-color:var(--accent)}.settings-content .settings-grid{grid-template-columns:1fr}.settings-actions{position:sticky;bottom:0;z-index:5;background:var(--panel);border-top:2px solid var(--line);padding:.75rem;margin-top:1rem}
.field{display:grid;gap:.38rem;margin:.7rem 0}
.field label{font-weight:750}
.field small{color:var(--muted)}
.field.safety{border-left:3px solid var(--warn);padding-left:.7rem}
.field-head{display:flex;align-items:center;justify-content:space-between;gap:.5rem}
.help-button{min-height:34px;padding:.25rem .6rem;font-weight:750}
.help-panel{background:var(--panel2);border:1px solid var(--line);border-radius:6px;padding:.65rem}
.help-panel p{margin:.2rem 0}
.setting-meta{color:var(--muted);font-size:.9rem}
.field-error{color:var(--danger);font-weight:700;min-height:1.2em}
input[aria-invalid=true],select[aria-invalid=true],textarea[aria-invalid=true]{border-color:var(--danger)}
input[type=number],input[type=text],input[type=password],select,textarea{width:100%;min-height:42px;padding:.5rem .58rem;background:var(--panel)}
.controls input,.controls select{width:auto;min-width:7rem}
...
.notice{border:1px solid var(--line);border-left:4px solid var(--accent);padding:.75rem .95rem;background:var(--panel);border-radius:6px}
.warning{border-left-color:var(--warn)}
.error{border-left-color:var(--danger)}
.report{background:var(--panel2);border:1px solid var(--line);border-radius:6px;padding:.75rem;margin-top:.75rem}
.report h3{margin:.1rem 0 .5rem}
.report-item{border-left:4px solid var(--line);padding:.35rem .55rem;margin:.35rem 0}
.report-error{border-left-color:var(--danger)}
.report-warn{border-left-color:var(--warn)}
.report-info{border-left-color:var(--accent)}
.report-ok{border-left-color:var(--good)}
.status-chip{display:inline-flex;align-items:center;gap:.4rem;border:1px solid var(--line);border-radius:4px;padding:.2rem .5rem;font-weight:800}
.status-chip.ok{border-color:var(--good)}.status-chip.ok:before{content:"●";color:var(--good)}
.status-chip.warn{border-color:var(--warn)}.status-chip.warn:before{content:"▲";color:var(--warn);font-size:.7em}
.status-chip.bad{border-color:var(--danger)}.status-chip.bad:before{content:"!";color:var(--danger)}
.drive-hero{display:grid;grid-template-columns:minmax(15rem,.8fr) minmax(0,1.4fr);gap:1rem;background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:1.25rem}.drive-primary{display:flex;align-items:center;gap:1.2rem}.platter{width:7.5rem;height:7.5rem;flex:0 0 auto;border:1rem solid var(--panel2);outline:2px solid var(--line);border-radius:50%;display:grid;place-items:center}.platter:after{content:"";width:1.2rem;height:1.2rem;border-radius:50%;background:var(--accent)}.drive-speed{font-size:2rem;letter-spacing:-.03em;margin:.15rem 0}.drive-readouts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.75rem}.readout{border-left:3px solid var(--line);padding:.3rem .75rem}.readout span{display:block;color:var(--muted);font-size:.8rem;text-transform:uppercase;font-weight:750}.readout strong{display:block;font-size:1.25rem}.dashboard-active>.wrap>.status-grid{display:none}
.dash-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:.85rem}
.dash-tile,.bench-card{background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:1rem;position:relative;overflow:hidden}
.dash-tile span,.bench-card span{color:var(--muted);display:block}
.dash-tile span:first-child,.bench-card span:first-child{font-weight:800;text-transform:uppercase;font-size:.82rem}
.dash-tile strong{font-size:1.62rem;display:block;line-height:1.1}
.bench-card textarea{min-height:12rem}
.progress{height:.72rem;background:var(--panel2);border:1px solid var(--line);overflow:hidden}
.progress i{display:block;height:100%;background:var(--accent-fill);width:0}
.scope{width:180px;height:180px;border:1px solid var(--line);border-radius:6px;background:var(--panel2)}
.chart{width:100%;height:240px;border:1px solid var(--line);border-radius:6px;background:var(--panel)}
.telemetry-grid{display:grid;grid-template-columns:minmax(0,2fr) minmax(220px,1fr);gap:1rem;align-items:start}
.legend{display:flex;flex-wrap:wrap;gap:.55rem;margin:.5rem 0}
.legend label{border:1px solid var(--line);border-radius:4px;padding:.3rem .68rem;background:var(--panel)}
.simple-controls button{min-width:8rem;min-height:64px}
.cal-card{display:none}
.cal-card.active-cal{display:block;grid-column:1/-1}
.preset-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:1rem}
.preset-card{background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:1rem}
.diff{white-space:pre-wrap;background:var(--panel2);border:1px solid var(--line);border-radius:6px;padding:.75rem;margin-top:.75rem;max-height:14rem;overflow:auto}
.log{white-space:pre-wrap;background:var(--panel2);border:1px solid var(--line);border-radius:6px;padding:1rem;min-height:8rem}
.hide{display:none!important}
@media (prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important;animation:none!important}}
@media (max-width:800px){.status-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.status-grid .metric:first-child{grid-column:1/-1}.telemetry-grid,.drive-hero,.settings-shell{grid-template-columns:1fr}.drive-primary{align-items:flex-start}.top{align-items:flex-start;flex-direction:column;gap:.55rem}.header-tools{display:grid;grid-template-columns:1fr 1fr;width:100%}.toolbar-field{width:auto;display:grid;grid-template-columns:auto minmax(0,1fr)}.toolbar-field select{width:100%}.header-tools>button{grid-column:1/-1}.tabs{align-items:stretch}.nav-more{width:100%}.nav-more summary{width:100%}.nav-menu{position:static;width:100%;margin-top:.35rem}.wrap{padding:.75rem}.metric strong{font-size:1.25rem}.controls,.button-row{display:grid;grid-template-columns:1fr 1fr}.controls input,.controls select{width:100%}.simple-controls .button-row{grid-template-columns:1fr}.settings-nav{position:static;display:flex;overflow-x:auto}.settings-nav a{min-width:max-content;border-left:0;border-bottom:3px solid transparent}.settings-nav a:hover,.settings-nav a:focus-visible{border-left:0;border-bottom-color:var(--accent)}.dash-grid,.bench-grid{grid-template-columns:1fr}.chart{height:220px}}
@media (max-width:430px){button{padding:.55rem .62rem}.controls,.button-row{grid-template-columns:1fr}.status-grid{grid-template-columns:1fr}.brand{font-size:1.32rem}.dash-tile strong,.metric strong{font-size:1.42rem}.platter{width:5rem;height:5rem;border-width:.65rem}.safety-inner button{min-width:9rem}}
</style>
</head>
<body>
<a class="skip" href="#mainContent">Skip to main controls</a>
<header><div class="top"><div><div class="brand">TT Control</div><div id="live" class="net" role="status">Loading</div></div><div><div class="net" id="networkLine"></div><div class="header-tools" aria-label="Web UI preferences"><label class="toolbar-field" for="homeSelect"><span>Home</span><select id="homeSelect"></select></label><label class="toolbar-field" for="themeSelect"><span>Theme</span><select id="themeSelect"><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option><option value="contrast">High contrast</option></select></label><button id="largeToggle" type="button">Large controls</button></div></div></div></header>
<div class="safety-bar" role="region" aria-label="Emergency control">
<div class="safety-inner"><div class="safety-state"><strong id="safetyState">Controller status</strong><span id="safetyDetail">Waiting for motor data</span></div><button class="danger" data-action="emergencyStop"><span class="ico" aria-hidden="true">!</span>Emergency stop</button></div>
</div>
<div class="wrap">
<div class="status-grid" role="region" aria-label="Current status">
...
<button id="tabDashboard" aria-controls="dashboard" aria-pressed="true" data-tab="dashboard">Dashboard</button>
<button id="tabControl" aria-controls="control" aria-pressed="false" data-tab="control">Control</button>
<button id="tabSettings" aria-controls="settings" aria-pressed="false" data-tab="settings">Tune</button>
<button id="tabPowerStage" aria-controls="powerstage" aria-pressed="false" data-tab="powerstage">Output status</button>
<button id="tabPresets" aria-controls="presets" aria-pressed="false" data-tab="presets">Presets</button>
<details class="nav-more" id="toolsNav"><summary>Tools &amp; system</summary><div class="nav-menu">
<button id="tabCalibrate" aria-controls="calibrate" aria-pressed="false" data-tab="calibrate">Calibration</button>
<button id="tabNetwork" aria-controls="network" aria-pressed="false" data-tab="network">Network</button>
<button id="tabBench" aria-controls="bench" aria-pressed="false" data-tab="bench">Bench test</button>
<button id="tabDiagnostics" aria-controls="diagnostics" aria-pressed="false" data-tab="diagnostics">Diagnostics</button>
<button id="tabErrors" aria-controls="errors" aria-pressed="false" data-tab="errors">Error log</button>
</div></details>
</nav>
<main id="mainContent" tabindex="-1">
<section id="dashboard" class="active" role="region" aria-labelledby="tabDashboard">
...
<button data-mode="cpu" aria-pressed="false">CPU</button>
<button data-mode="memory" aria-pressed="false">Memory</button>
<button data-mode="flash" aria-pressed="false">Flash</button>
</div>
<div id="dashboardBody" class="panel"></div>
</section>
<section id="control" role="region" aria-labelledby="tabControl" hidden>
<div class="panel simple-controls">
<h2>Simple control</h2>
<div class="button-row"><button class="good" data-action="start"><span class="ico" aria-hidden="true">RUN</span>Start</button><button class="danger" data-action="emergencyStop"><span class="ico" aria-hidden="true">!</span>Emergency stop</button><button class="danger" data-action="stop"><span class="ico" aria-hidden="true">STP</span>Stop</button><button data-action="toggleStandby"><span class="ico" aria-hidden="true">PWR</span>Standby / wake</button></div>
<div class="button-row"><button data-speed="0"><span class="ico" aria-hidden="true">33</span>33 RPM</button><button data-speed="1"><span class="ico" aria-hidden="true">45</span>45 RPM</button><button data-speed="2"><span class="ico" aria-hidden="true">78</span>78 RPM</button><button data-action="resetPitch"><span class="ico" aria-hidden="true">PIT</span>Reset pitch</button></div>
...
<section id="settings" role="region" aria-labelledby="tabSettings" hidden>
...
<div class="settings-shell"><nav id="settingsNav" class="settings-nav" aria-label="Settings sections"></nav><div class="settings-content">
<div id="globalSettings" class="settings-grid"></div>
<div class="panel" id="speed-settings-section">
<h2>Speed settings</h2>
<div class="speed-tabs" role="tablist" aria-label="Speed settings"><button id="speedTab33" role="tab" aria-controls="speedSettings" data-speed-tab="0" aria-selected="true" tabindex="0">33 RPM</button><button id="speedTab45" role="tab" aria-controls="speedSettings" data-speed-tab="1" aria-selected="false" tabindex="-1">45 RPM</button><button id="speedTab78" role="tab" aria-controls="speedSettings" data-speed-tab="2" aria-selected="false" tabindex="-1">78 RPM</button></div>
<div id="speedSettings" class="settings-grid"></div>
</div>
</div></div>
<div class="settings-actions"><div class="button-row"><button class="primary" id="applySave">Save settings</button><button id="applyOnly">Apply without saving</button><button id="reviewSettings">Review changes</button><button id="discardSettings">Discard changes</button><button id="resetRuntime">Reset runtime</button><button class="danger" id="factoryReset">Factory reset</button></div></div>
</section>
<section id="powerstage" role="region" aria-labelledby="tabPowerStage" hidden>
<div id="powerStageBody"></div>
</section>
<section id="calibrate" role="region" aria-labelledby="tabCalibrate" hidden>
<div class="panel section-head">
...
<section id="network" role="region" aria-labelledby="tabNetwork" hidden>
...
<div class="panel"><h2>Saved credentials</h2><p class="net">Blank password fields keep the existing password. Use these controls only when you intend to remove a saved credential.</p><label class="check-row"><input id="clearStationPassword" type="checkbox"> Remove saved Wi-Fi password</label><label class="check-row"><input id="clearApPassword" type="checkbox"> Remove setup AP password and make it open</label></div>
<div id="scanResults" class="panel"></div>
</section>
<section id="presets" role="region" aria-labelledby="tabPresets" hidden>
...
<section id="bench" role="region" aria-labelledby="tabBench" hidden>
<div id="benchBody"></div>
</section>
<section id="diagnostics" role="region" aria-labelledby="tabDiagnostics" hidden>
...
<section id="errors" role="region" aria-labelledby="tabErrors" hidden>
...
let activeCalStep=0;
const telemetrySeries={frequency:true,pitch:true,amp:true,rpm:true};
...
const tabNames={dashboard:"Dashboard",control:"Control",settings:"Settings",powerstage:"Output status",calibrate:"Calibrate",network:"Network",presets:"Presets",bench:"Bench",diagnostics:"Diagnostics",errors:"Errors"};
const speedFieldGroups=[
["Frequency",["frequency","minFrequency","maxFrequency","filterType","iirAlpha","firProfile"]],
["Phase",["phase0","phase1","phase2","phase3"]],
["Startup",["softStartDuration","startupKick","startupKickDuration","startupKickRampDuration"]],
["Amplitude",["reducedAmplitude","amplitudeDelay","gain0","gain1","gain2","gain3"]],
["Closed-loop tuning",["closedLoopDeadbandRpm","closedLoopLockToleranceRpm","closedLoopLockTimeMs","closedLoopKp","closedLoopKi","closedLoopKd","closedLoopIntegralLimitHz","closedLoopCorrectionLimitHz","closedLoopSlewLimitHzPerSec","closedLoopRampKp","closedLoopRampCorrectionLimitHz"]]
];
const networkFieldGroups=[
["Connection",["enabled","mode","hostname","standbyMode"]],
...
function isEcoStandbyMode(){return Number(networkData?.config?.standbyMode ?? statusData?.network?.standbyMode ?? 0)===1}
function isStandbyActive(){return !!(statusData&&statusData.motor&&statusData.motor.standby)}
...
function renderAuth(){const p=$("authPanel");if(!p||!authState){return}if(!authState.required){p.classList.add("hide");setLockedUI();return}p.classList.remove("hide");const unlocked=!!authState.unlocked;$("authTitle").textContent=unlocked?"Controls unlocked":"PIN required: enter PIN to change controls or settings";$("unlockWeb").hidden=unlocked;$("pinInput").hidden=unlocked;document.querySelector('label[for="pinInput"]').hidden=unlocked;$("lockWeb").hidden=!unlocked;setLockedUI()}
...
function fieldGroups(fields,groups){const used=new Set(),out=[];for(const [title,keys] of groups){const list=[];for(const key of keys){const f=findField(fields,key);if(f){list.push(f);used.add(f.k)}}if(list.length)out.push([title,list])}const remaining=fields.filter(f=>!used.has(f.k));if(remaining.length)out.push(["Other",remaining]);return out}
function appendFieldGroup(root,legend,fields,scope,source,index){const p=document.createElement("fieldset");p.className="panel";p.innerHTML=`<legend>${esc(legend)}</legend>`;for(const f of fields){let value;if(scope==="speed")value=f.k.startsWith("phase")?source.phaseOffset[Number(f.k.slice(5))]:f.k.startsWith("gain")?source.channelAmplitude[Number(f.k.slice(4))]:source[f.k];else{value=source[f.k];if(f.t==="password")value=""}p.appendChild(makeField(scope,f,value,index))}root.appendChild(p)}
function optionLabel(key,value){const found=(options[key]||[]).find(o=>Number(o[0])===Number(value));return found?found[1]:String(value)}
function pathLabel(path){const parts=path.split(".");if(parts[0]==="global"){const f=findField(globalFields(),parts[1]);return f?f.l:path}if(parts[0]==="network"){if(parts[1]==="passwordSet")return "Wi-Fi password state";if(parts[1]==="apPasswordSet")return "Setup AP password state";const f=findField(networkFields,parts[1]);return f?f.l:path}if(parts[0]==="speeds"){const index=Number(parts[1]);if(parts[2]==="phaseOffset")return `${speedNames[index]} phase ${Number(parts[3])+1} offset`;const f=findField(speedFields,parts[2]);return f?`${speedNames[index]} ${f.l}`:path}return path}
...
function speedComparable(s){const out={phaseOffset:[...s.phaseOffset],channelAmplitude:[...s.channelAmplitude]};speedFields.forEach(f=>{if(!f.k.startsWith("phase")&&!f.k.startsWith("gain"))out[f.k]=s[f.k]});return out}
...
function networkComparableFromConfig(c){const out={};networkFields.forEach(f=>{if(f.k!=="password"&&f.k!=="apPassword")out[f.k]=c[f.k]});out.passwordSet=!!c.passwordSet;out.apPasswordSet=!!c.apPasswordSet;return out}
function currentNetworkComparable(){const out=gatherNetwork(),station=$(inputId("network","password"))?.value||"",ap=$(inputId("network","apPassword"))?.value||"";delete out.password;delete out.apPassword;delete out.clearPassword;delete out.clearApPassword;out.passwordSet=station?true:$('clearStationPassword')?.checked?false:!!baselineNetwork?.passwordSet;out.apPasswordSet=ap?true:$('clearApPassword')?.checked?false:!!baselineNetwork?.apPasswordSet;return out}
...
function renderSettings(){if(!settingsData)return;const root=$("globalSettings"),nav=$("settingsNav");root.innerHTML="";nav.innerHTML="";globalGroups.forEach(([title,fields],index)=>{const p=document.createElement("fieldset"),id=`setting-group-${index}`;p.className="panel";p.id=id;p.innerHTML=`<legend>${esc(title)}</legend>`;for(const f of fields)p.appendChild(makeField("global",f,settingsData.global[f.k]));root.appendChild(p);const a=document.createElement("a");a.href=`#${id}`;a.textContent=title;nav.appendChild(a)});const speedLink=document.createElement("a");speedLink.href="#speed-settings-section";speedLink.textContent="Speed settings";nav.appendChild(speedLink);renderSpeedSettings();wireForm($("settings"),()=>{validateSettingsForm(false);updateSettingsDirty()});updateSettingsDirty();filterSettings();setLockedUI()}
...
function captureSpeedTab(){if(!settingsData||!$("speedSettings").children.length)return;const s=settingsData.speeds[speedTab];for(const f of speedFields){if(f.k.startsWith("phase"))s.phaseOffset[Number(f.k.slice(5))]=fieldValue("speed",f,speedTab);else if(f.k.startsWith("gain"))s.channelAmplitude[Number(f.k.slice(4))]=fieldValue("speed",f,speedTab);else s[f.k]=fieldValue("speed",f,speedTab)}}
...
function gatherNetwork(){const out={};for(const f of networkFields){const v=fieldValue("network",f);if((f.k==="password"||f.k==="apPassword")&&v==="")continue;out[f.k]=v}if($("clearStationPassword")?.checked)out.clearPassword=true;if($("clearApPassword")?.checked)out.clearApPassword=true;return out}
...
function drawTelemetry(){const c=$("telemetryChart");if(!c||!telemetry.length)return;document.querySelectorAll("[data-series]").forEach(el=>el.onchange=()=>{telemetrySeries[el.dataset.series]=el.checked;drawTelemetry()});const ctx=c.getContext("2d"),style=getComputedStyle(document.body),w=720,h=240,pad=32;ctx.clearRect(0,0,w,h);ctx.strokeStyle=style.getPropertyValue("--line");ctx.lineWidth=1;for(let i=0;i<=4;i++){const y=20+i*42;ctx.beginPath();ctx.moveTo(pad,y);ctx.lineTo(w-pad,y);ctx.stroke()}for(let i=0;i<=6;i++){const x=pad+i*(w-pad*2)/6;ctx.beginPath();ctx.moveTo(x,20);ctx.lineTo(x,188);ctx.stroke()}ctx.strokeRect(pad,20,w-pad*2,168);ctx.fillStyle=style.getPropertyValue("--muted");ctx.fillText("newer",w-pad-38,214);ctx.fillText("older",pad,214);const f=telemetry.map(x=>x.f),p=telemetry.map(x=>x.p),a=telemetry.filter(x=>x.a!==null).map(x=>x.a),rpm=telemetry.filter(x=>x.rpm!==null).map(x=>x.rpm);if(telemetrySeries.frequency)drawSeries(ctx,f,style.getPropertyValue("--accent"),Math.min(...f),Math.max(...f));if(telemetrySeries.pitch)drawSeries(ctx,p,style.getPropertyValue("--good"),-50,50);if(telemetrySeries.amp&&a.length)drawSeries(ctx,a,style.getPropertyValue("--warn"),20,90);if(telemetrySeries.rpm&&rpm.length)drawSeries(ctx,rpm,style.getPropertyValue("--danger"),0,120);let lx=pad;[["frequency","Frequency","--accent"],["pitch","Pitch","--good"],["amp","Amp temp","--warn"],["rpm","Measured RPM","--danger"]].forEach(([k,l,cvar])=>{if(!telemetrySeries[k])return;ctx.fillStyle=style.getPropertyValue(cvar);ctx.fillRect(lx,222,12,4);ctx.fillStyle=style.getPropertyValue("--muted");ctx.fillText(l,lx+16,228);lx+=k==="rpm"?130:95})}
function bytesText(v){v=Number(v||0);return v>=1048576?(v/1048576).toFixed(1)+" MB":Math.round(v/1024)+" KB"}
function pctText(used,total){used=Number(used||0);total=Number(total||0);return total?Math.round(used*100/total)+"%":"0%"}
function dashboardOptionEnabled(mode){const g=settingsData?.global||{};if(mode==="cpu")return g.showCpuDashboard!==false;if(mode==="memory")return g.showMemoryDashboard!==false;if(mode==="flash")return g.showFlashDashboard!==false;return true}
function syncDashboardModeButtons(){document.querySelectorAll("[data-mode]").forEach(b=>{const on=dashboardOptionEnabled(b.dataset.mode);b.hidden=!on;if(!on&&dashboardMode===b.dataset.mode)dashboardMode="standard"});document.querySelectorAll("[data-mode]").forEach(b=>b.setAttribute("aria-pressed",String(b.dataset.mode===dashboardMode)))}
function systemDashboardHtml(mode){const s=statusData.system||{},cpu=s.cpu||{},mem=s.memory||{},flash=s.flash||{};if(mode==="cpu")return `<h2>CPU</h2><div class="dash-grid"><div class="dash-tile"><span>Core 0</span><strong>${Number(cpu.core0Percent||0).toFixed(0)}%</strong></div><div class="dash-tile"><span>Waveform core</span><strong>${Number(cpu.waveformPercent||0).toFixed(0)}%</strong></div></div>`;if(mode==="memory")return `<h2>Memory</h2><div class="dash-grid"><div class="dash-tile"><span>Heap used</span><strong>${bytesText(mem.heapUsedBytes)}</strong><div class="progress"><i style="width:${pctText(mem.heapUsedBytes,mem.heapTotalBytes)}"></i></div></div><div class="dash-tile"><span>Heap free</span><strong>${bytesText(mem.heapFreeBytes)}</strong></div><div class="dash-tile"><span>PSRAM used</span><strong>${bytesText(mem.psramUsedBytes)}</strong></div><div class="dash-tile"><span>PSRAM free</span><strong>${bytesText(mem.psramFreeBytes)}</strong></div></div>`;return `<h2>Flash</h2><div class="dash-grid"><div class="dash-tile"><span>Flash total</span><strong>${bytesText(flash.totalBytes)}</strong></div><div class="dash-tile"><span>Sketch</span><strong>${bytesText(flash.sketchUsedBytes)} / ${bytesText(flash.sketchCapacityBytes)}</strong><div class="progress"><i style="width:${pctText(flash.sketchUsedBytes,flash.sketchCapacityBytes)}"></i></div></div><div class="dash-tile"><span>Filesystem</span><strong>${flash.filesystemMounted?`${bytesText(flash.filesystemUsedBytes)} / ${bytesText(flash.filesystemTotalBytes)}`:"not mounted"}</strong><div class="progress"><i style="width:${pctText(flash.filesystemUsedBytes,flash.filesystemTotalBytes)}"></i></div></div></div>`}
function renderDashboard(){if(!statusData||currentTab!=="dashboard")return;syncDashboardModeButtons();const b=$("dashboardBody");if(b.contains(document.activeElement))return;const m=statusData.motor,amp=statusData.amp.enabled?`${statusData.amp.temperatureC.toFixed(1)} C`:"Off",cls=statusClass(m.state),motion=Math.round(m.motionProgress*100),name=speedNames[m.speed]||m.speedName||"-";const hero=`<div class="drive-hero"><div class="drive-primary"><div class="platter" aria-hidden="true"></div><div><span class="status-chip ${cls}">${esc(m.state)}</span><div class="drive-speed">${esc(name)}</div><span class="net">Selected speed</span></div></div><div class="drive-readouts"><div class="readout"><span>Frequency</span><strong>${m.frequency.toFixed(2)} Hz</strong></div><div class="readout"><span>Pitch</span><strong>${m.pitch.toFixed(2)}%</strong></div><div class="readout"><span>Motion</span><strong>${motion}%</strong></div><div class="readout"><span>Amplifier</span><strong>${esc(amp)}</strong></div></div></div>`;const clTile=closedLoopTileHtml(m.closedLoop);if(["cpu","memory","flash"].includes(dashboardMode)){b.innerHTML=systemDashboardHtml(dashboardMode);return}if(dashboardMode==="dim"){b.innerHTML=hero;return}if(dashboardMode==="stats"){b.innerHTML=`${hero}<h2>Runtime and telemetry</h2><div class="dash-grid"><div class="dash-tile"><span>Session</span><strong>${statusData.runtime.session}s</strong></div><div class="dash-tile"><span>Total</span><strong>${statusData.runtime.total}s</strong></div>${clTile}</div>${telemetryHtml()}`;drawTelemetry();return}if(dashboardMode==="scope"){b.innerHTML=`${hero}<h2>Phase scope</h2><canvas class="scope" id="scopeCanvas" width="180" height="180" role="img" aria-label="Phase A and B position; numeric values follow"></canvas><p>Phase A: ${statusData.scope.a}, Phase B: ${statusData.scope.b}</p>${telemetryHtml()}`;const c=$("scopeCanvas"),x=c.getContext("2d"),style=getComputedStyle(document.body);x.clearRect(0,0,180,180);x.strokeStyle=style.getPropertyValue("--line");x.strokeRect(20,20,140,140);x.fillStyle=style.getPropertyValue("--accent");x.beginPath();x.arc(90+statusData.scope.a/8,90-statusData.scope.b/8,5,0,Math.PI*2);x.fill();drawTelemetry();return}b.innerHTML=`${hero}${clTile?`<div class="dash-grid">${clTile}</div>`:""}${telemetryHtml()}`;drawTelemetry()}
function renderStatus(){if(!statusData)return;const m=statusData.motor,ampText=statusData.amp.enabled?(statusData.amp.thermalOk?"OK":"TRIPPED"):"Off",name=speedNames[m.speed]||m.speedName||"-";if(lastState&&lastState!==m.state)addEvent(`Motor state ${m.state}`);if(lastSpeed>=0&&lastSpeed!==m.speed)addEvent(`Speed ${name}`);if(lastAmpState&&lastAmpState!==ampText)addEvent(`Amplifier ${ampText}`);lastState=m.state;lastSpeed=m.speed;lastAmpState=ampText;$("state").textContent=m.state;$("speed").textContent=name;$("frequency").textContent=m.frequency.toFixed(2)+" Hz";$("pitch").textContent=m.pitch.toFixed(2)+"%";$("ampState").textContent=ampText;$("safetyState").textContent=m.state;$("safetyDetail").textContent=`${name} · ${m.frequency.toFixed(2)} Hz · pitch ${m.pitch.toFixed(2)}%`;const cls=statusClass(m.state);$("state").closest(".metric").className=`metric ${cls}`;$("speed").closest(".metric").className=`metric ${m.running?"ok":"warn"}`;$("frequency").closest(".metric").className=`metric ${m.speedRamping?"warn":m.running?"ok":""}`;$("pitch").closest(".metric").className=`metric ${Math.abs(m.pitch)>0.01?"warn":""}`;$("ampState").closest(".metric").className=`metric ${ampText==="TRIPPED"?"bad":ampText==="OK"?"ok":""}`;if($("simplePitch")&&document.activeElement!==$("simplePitch"))$("simplePitch").value=m.pitch.toFixed(1);pushTelemetry();renderDashboard();renderBench();setLockedUI()}
function renderPowerStage(){if(!statusData||currentTab!=="powerstage")return;const root=$("powerStageBody"),m=statusData.motor,o=m.output||{},metrics=o.metrics||{},count=Math.max(1,Math.min(4,Number(settingsData?.global?.phaseMode||o.channels?.length||3))),channels=(o.channels||[]).slice(0,count),cx=120,cy=120,r=82;const colours=["var(--accent)","var(--good)","var(--warn)","var(--danger)"];let svg=`<svg viewBox="0 0 240 240" role="img" aria-label="Applied phase vectors" style="width:min(100%,24rem);height:auto"><circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="var(--line)"/>`;channels.forEach((c,i)=>{const a=Number(c.appliedPhase||0)*Math.PI/180,len=r*Math.max(.1,Number(c.appliedGain||100)/150),x=cx+Math.cos(a)*len,y=cy-Math.sin(a)*len;svg+=`<line x1="${cx}" y1="${cy}" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}" stroke="${colours[i]}" stroke-width="4"/><text x="${x.toFixed(1)}" y="${y.toFixed(1)}" fill="var(--text)">${String.fromCharCode(65+i)}</text>`});svg+="</svg>";const rows=channels.map((c,i)=>`<tr><th>${String.fromCharCode(65+i)}</th><td>${Number(c.targetPhase||0).toFixed(1)}°</td><td>${Number(c.appliedPhase||0).toFixed(1)}°</td><td>${Number(c.targetGain||0)}%</td><td>${Number(c.appliedGain||0).toFixed(1)}%</td><td>${Number(c.headroom||0).toFixed(1)}%</td><td>${Number(c.clippingCount||0)}</td></tr>`).join("");const fault=o.lastFault||{};root.innerHTML=`<div class="panel section-head"><h2>Power stage</h2><p><span class="status-chip ${m.driverFault?"bad":m.driverEnabled?"ok":"warn"}">${esc(o.state||m.outputBackend)}</span> ${esc(m.outputBackend||"")}</p></div><div class="settings-grid"><div class="panel"><h3>Phase vectors</h3>${svg}</div><div class="panel"><h3>Lifecycle counters</h3><p>Successful enables: ${Number(metrics.successfulEnables||0)} / ${Number(metrics.enableAttempts||0)}</p><p>Faults: ${Number(metrics.faultCount||0)} total, ${Number(metrics.wakeFaultCount||0)} during wake, ${Number(metrics.runningFaultCount||0)} while running.</p></div></div><div class="panel"><h3>Live modulation</h3><div style="overflow:auto"><table><thead><tr><th>Channel</th><th>Target phase</th><th>Applied phase</th><th>Target gain</th><th>Applied gain</th><th>Headroom</th><th>Clips</th></tr></thead><tbody>${rows}</tbody></table></div></div><div class="panel"><h3>Last driver fault</h3><p>${fault.valid?`At ${Number(fault.timestampMs)} ms; origin state ${Number(fault.originState)}, motor state ${Number(fault.motorState)}, speed ${Number(fault.speed)}, ${Number(fault.frequencyHz||0).toFixed(2)} Hz, waveform buffer ${Number(fault.bufferFillCount||0)}.`:"No driver fault snapshot recorded this boot."}</p></div>`}
function adaptOutputStatus(){
if(!statusData)return;
const bridge=String(statusData.motor?.outputBackend||"").toLowerCase().includes("bridge"),label=bridge?"Driver status":"Output status";
tabNames.powerstage=label;
const tab=$("tabPowerStage");if(tab)tab.textContent=label;
if(currentTab!=="powerstage")return;
const root=$("powerStageBody"),heading=root?.querySelector(".section-head h2"),summary=root?.querySelector(".section-head p");
if(heading)heading.textContent=label;
if(!bridge&&summary)summary.textContent=statusData.motor.outputBackend||"Linear output";
if(!bridge&&root){
[...root.querySelectorAll(".panel")].forEach(panel=>{const title=panel.querySelector("h3")?.textContent;if(title==="Lifecycle counters"||title==="Last driver fault")panel.remove()});
}
}
async function loadStatus(){statusData=await api("/api/status");renderStatus();renderPowerStage();adaptOutputStatus()}
function startStatusStream(){if(!("EventSource" in window)){setInterval(loadStatus,1000);return}let fallback=false;const es=new EventSource("/api/events");es.addEventListener("status",e=>{try{statusData=JSON.parse(e.data);renderStatus();renderPowerStage();adaptOutputStatus()}catch(err){}});es.onerror=()=>{if(!fallback&&!telemetry.length){fallback=true;es.close();setInterval(loadStatus,1000)}}}
...
const relaySelect=$("benchRelayStage");
if(relaySelect){
if(Number(m.relayStageCount||0)>0)relaySelect.value=String(m.relayStage||0);
else{
relaySelect.closest(".bench-card")?.remove();
[...root.querySelectorAll(".dash-tile")].find(el=>el.querySelector("span")?.textContent==="Relay test")?.remove();
}
}
$("benchRefresh").onclick=()=>loadDiagnostics().catch(e=>setLive(e.message));
$("benchSetPitch").onclick=()=>control("setPitch",{pitch:Number($("benchPitch").value)}).catch(e=>setLive(e.message));
$("benchMakeReport").onclick=()=>{$("benchReport").value=benchReportText()};
document.querySelectorAll("[data-bench]").forEach(b=>b.onclick=()=>{const action=b.dataset.bench;if((action==="start"||action==="relayTest")&&!confirm("Run this bench test action now?"))return;const extra=action==="relayTest"?{stage:Number($("benchRelayStage").value)}:{};control(action,extra).catch(e=>setLive(e.message))});
document.querySelectorAll("[data-bench-speed]").forEach(b=>b.onclick=()=>setSpeedControl(b.dataset.benchSpeed).catch(e=>setLive(e.message)));
setLockedUI();
}
...
function applyTheme(theme){if(!["system","light","dark","contrast"].includes(theme))theme="system";document.body.dataset.theme=theme;try{localStorage.setItem("ttc_theme",theme)}catch(e){}const el=$("themeSelect");if(el)el.value=theme}
...
function loadTabContent(tab){if(tab==="network")loadNetwork().catch(e=>setLive(e.message));if(tab==="presets")loadPresets().catch(e=>setLive(e.message));if(tab==="bench")loadBench().catch(e=>setLive(e.message));if(tab==="diagnostics")loadDiagnostics().catch(e=>setLive(e.message));if(tab==="errors")loadErrors().catch(e=>setLive(e.message));if(tab==="calibrate")renderCalibrationTools();if(tab==="powerstage"){renderPowerStage();adaptOutputStatus()}}
function activateTab(tab,focus=false){if(!$(tab))tab="dashboard";currentTab=tab;const secondary=["calibrate","network","bench","diagnostics","errors"],tools=$("toolsNav");document.body.classList.toggle("dashboard-active",tab==="dashboard");document.querySelectorAll("[data-tab]").forEach(x=>x.setAttribute("aria-pressed",String(x.dataset.tab===tab)));if(tools){tools.open=secondary.includes(tab);tools.querySelector("summary").setAttribute("aria-label",secondary.includes(tab)?`Tools and system, current page ${tabNames[tab]}`:"Tools and system")}document.querySelectorAll("main section").forEach(s=>{const active=s.id===tab;s.classList.toggle("active",active);s.hidden=!active;s.tabIndex=-1});if(focus)$(tab).focus();loadTabContent(tab);if(tab==="dashboard")renderDashboard()}
function handleTabKey(e){const tabs=[...document.querySelectorAll("[data-tab]")],i=tabs.indexOf(e.currentTarget);let next=-1;if(e.key==="ArrowRight"||e.key==="ArrowDown")next=(i+1)%tabs.length;else if(e.key==="ArrowLeft"||e.key==="ArrowUp")next=(i+tabs.length-1)%tabs.length;else if(e.key==="Home")next=0;else if(e.key==="End")next=tabs.length-1;if(next<0)return;e.preventDefault();activateTab(tabs[next].dataset.tab,false);tabs[next].focus()}
function selectSpeedTab(next,focus=false){next=Number(next);if(next===2&&!is78Enabled()){const msg=disabled78Message();alert(msg);setLive(msg);return}captureSpeedTab();speedTab=next;renderSpeedSettings();const btn=document.querySelector(`[data-speed-tab="${speedTab}"]`);if(focus&&btn)btn.focus()}
function handleSpeedTabKey(e){const tabs=[...document.querySelectorAll("[data-speed-tab]")].filter(b=>!b.hidden),i=tabs.indexOf(e.currentTarget);let next=-1;if(e.key==="ArrowRight"||e.key==="ArrowDown")next=(i+1)%tabs.length;else if(e.key==="ArrowLeft"||e.key==="ArrowUp")next=(i+tabs.length-1)%tabs.length;else if(e.key==="Home")next=0;else if(e.key==="End")next=tabs.length-1;if(next<0)return;e.preventDefault();selectSpeedTab(tabs[next].dataset.speedTab,true)}
function wire(){initPrefs();try{authToken=sessionStorage.getItem("ttc_token")||""}catch(e){}document.querySelectorAll("[data-tab]").forEach(b=>{b.onclick=()=>activateTab(b.dataset.tab);b.onkeydown=handleTabKey});document.querySelectorAll("[data-mode]").forEach(b=>b.onclick=()=>{dashboardMode=b.dataset.mode;document.querySelectorAll("[data-mode]").forEach(x=>x.setAttribute("aria-pressed",String(x===b)));renderDashboard()});document.querySelectorAll("[data-action]").forEach(b=>b.onclick=()=>control(b.dataset.action).catch(e=>setLive(e.message)));document.querySelectorAll("[data-speed]").forEach(b=>b.onclick=()=>setSpeedControl(b.dataset.speed).catch(e=>setLive(e.message)));document.querySelectorAll("[data-speed-tab]").forEach(b=>{b.onclick=()=>selectSpeedTab(b.dataset.speedTab);b.onkeydown=handleSpeedTabKey});$("unlockWeb").onclick=()=>unlockWeb().catch(e=>setLive(e.message));$("lockWeb").onclick=()=>lockWeb().catch(e=>setLive(e.message));$("setSimplePitch").onclick=()=>control("setPitch",{pitch:Number($("simplePitch").value)}).catch(e=>setLive(e.message));$("settingsSearch").oninput=filterSettings;$("reviewSettings").onclick=reviewSettingsChanges;$("applySave").onclick=()=>applySettings(true).catch(e=>setLive(e.message));$("applyOnly").onclick=()=>applySettings(false).catch(e=>setLive(e.message));$("discardSettings").onclick=loadSettings;$("factoryReset").onclick=()=>{if(confirm("Factory reset all settings and presets?"))control("factoryReset").then(()=>Promise.all([loadSettings(),loadNetwork()])).catch(e=>setLive(e.message))};$("resetRuntime").onclick=()=>control("resetRuntime").catch(e=>setLive(e.message));$("saveNetwork").onclick=()=>saveNetwork().catch(e=>setLive(e.message));$("discardNetwork").onclick=loadNetwork;$("scanNetworks").onclick=()=>scanNetworks(true).catch(e=>setLive(e.message));$("comparePresets").onclick=()=>comparePresetSlots().catch(e=>setLive(e.message));$("exportBackup").onclick=()=>exportBackup().catch(e=>setLive(e.message));$("validateBackup").onclick=()=>validateBackupText();$("importBackup").onclick=()=>importBackup().catch(e=>setLive(e.message));$("clearErrors").onclick=()=>api("/api/errors",{method:"POST"}).then(()=>{addEvent("Error log cleared");loadErrors()}).catch(e=>setLive(e.message))}
async function start(){document.body.setAttribute("aria-busy","true");wire();try{await loadSchema();const results=await Promise.allSettled([loadAuth(),loadStatus(),loadSettings(),loadNetwork()]),failed=results.filter(r=>r.status==="rejected");if(failed.length)setLive(`${failed.length} section${failed.length===1?"":"s"} could not be loaded`);else setLive("Web UI ready");activateTab(deviceHomeTab(),false);startStatusStream();setInterval(()=>loadAuth().catch(()=>{}),10000)}finally{document.body.setAttribute("aria-busy","false")}}
...
<style>
:root{color-scheme:light dark;--bg:#ecefeb;--panel:#fff;--text:#17201c;--muted:#59645e;--line:#c6cec9;--accent:#1b665e;--accent-fill:#185e57;--accent-text:#fff;--danger:#a52b24;--danger-text:#fff;--focus:#d47c00}
@media (prefers-color-scheme:dark){:root{--bg:#111613;--panel:#1a211d;--text:#f2f5f1;--muted:#b8c2bc;--line:#465249;--accent:#83d8cb;--accent-fill:#17635b;--accent-text:#fff;--danger:#a83d35;--danger-text:#fff;--focus:#ffd166}}
*{box-sizing:border-box}[hidden]{display:none!important}body{margin:0;background:var(--bg);color:var(--text);font:18px/1.5 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.wrap{max-width:820px;margin:0 auto;padding:1rem}header,.panel{background:var(--panel);border:1px solid var(--line);border-radius:6px;padding:1rem;margin:1rem 0}h1{font-size:1.6rem;margin:.2rem 0}.muted{color:var(--muted)}label{display:block;font-weight:700;margin:.8rem 0 .25rem}input,select,button{font:inherit;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--text)}input,select{width:100%;min-height:50px;padding:.45rem .6rem}button{min-height:50px;padding:.55rem .9rem;margin:.25rem .25rem .25rem 0;cursor:pointer;font-weight:700}button:hover:not(:disabled){border-color:var(--accent)}button.primary{background:var(--accent-fill);border-color:var(--accent-fill);color:var(--accent-text)}button.danger{background:var(--danger);border-color:var(--danger);color:var(--danger-text)}button:focus-visible,input:focus-visible,select:focus-visible{outline:4px solid var(--focus);outline-offset:2px}.check{display:flex;align-items:center;gap:.6rem}.check input{width:1.2rem}.networks button{display:block;width:100%;text-align:left}.notice{border-left:4px solid var(--accent)}.steps{display:grid;grid-template-columns:repeat(3,1fr);gap:.4rem}.steps button[aria-current=true]{border-color:var(--accent);background:var(--bg)}.field-error{color:var(--danger);font-weight:700;min-height:1.2em}.row{display:grid;grid-template-columns:1fr 1fr;gap:.75rem}.button-row{display:flex;flex-wrap:wrap;gap:.5rem}.summary{white-space:pre-wrap;background:var(--bg);border:1px solid var(--line);border-radius:6px;padding:.75rem}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important;animation:none!important}}@media(max-width:700px){.steps,.row{grid-template-columns:1fr}}
</style>
</head>
<body>
<div class="wrap">
...
<label class="check"><input id="hiddenSsid" type="checkbox"> Hidden network</label>
<label for="password">Wi-Fi password</label><input id="password" type="password" autocomplete="new-password">
<label class="check"><input id="clearPassword" type="checkbox"> Remove the saved Wi-Fi password</label>
...
<label for="apPassword">Setup network password</label><input id="apPassword" type="password" autocomplete="new-password" aria-describedby="apHelp"><p id="apHelp" class="muted">Blank keeps the saved password. Open setup mode only allows Wi-Fi configuration.</p><div id="apPasswordErr" class="field-error" aria-live="polite"></div>
<label class="check"><input id="clearApPassword" type="checkbox"> Remove the saved setup password and make the setup network open</label>
...
function updateSummary(){if(!$("summary"))return;const apSecurity=$("clearApPassword").checked?"open":$("apPassword").value?"new password":"keep saved setting";$("summary").textContent=`Mode: ${$("mode").selectedOptions[0]?.textContent||""}\nHostname: ${$("hostname").value||"ttcontrol"}\nWi-Fi: ${$("ssid").value||"not set"}${$("hiddenSsid").checked?" (hidden)":""}\nAddress: ${$("dhcp").checked?"DHCP":$("staticIp").value}\nSetup AP: ${$("apSsid").value||"TTControl-Setup"}\nSetup AP security: ${apSecurity}`}
...
function validate(n=step){let ok=true;err("ssidErr","");err("apSsidErr","");err("apPasswordErr","");err("apChannelErr","");["staticIp","gateway","subnet","dns"].forEach(k=>err(`${k}Err`,""));if(n===0&&Number($("mode").value)!==0&&!$("ssid").value.trim())ok=err("ssidErr","Wi-Fi network name is required for station mode.")&&ok;if(n===1&&!$("dhcp").checked){["staticIp","gateway","subnet","dns"].forEach(k=>{if(!validIp($(k).value.trim()))ok=err(`${k}Err`,"Enter a valid IPv4 address.")&&ok})}if(n===2){if(!$("apSsid").value.trim())ok=err("apSsidErr","Setup network name is required.")&&ok;if($("apPassword").value&&$("apPassword").value.length<8)ok=err("apPasswordErr","Use at least 8 characters, leave blank to keep the saved password, or select remove.")&&ok;const ch=Number($("apChannel").value);if(ch<1||ch>13)ok=err("apChannelErr","Choose channel 1 through 13.")&&ok}return ok}
async function load(){const n=await api("/api/network");setStatus(n);const c=n.config;$("enabled").checked=c.enabled;$("mode").value=c.mode;$("hostname").value=c.hostname;$("ssid").value=c.ssid;$("hiddenSsid").checked=!!c.hiddenSsid;$("password").value="";$("clearPassword").checked=false;$("clearPassword").disabled=!c.passwordSet;$("dhcp").checked=c.dhcp;$("staticIp").value=c.staticIp;$("gateway").value=c.gateway;$("subnet").value=c.subnet;$("dns").value=c.dns;$("apFallback").checked=c.apFallback;$("apSsid").value=c.apSsid;$("apPassword").value="";$("clearApPassword").checked=false;$("clearApPassword").disabled=!c.apPasswordSet;$("apChannel").value=c.apChannel;updateSummary()}
function body(){const out={enabled:$("enabled").checked,mode:Number($("mode").value),hostname:$("hostname").value,ssid:$("ssid").value,hiddenSsid:$("hiddenSsid").checked,dhcp:$("dhcp").checked,staticIp:$("staticIp").value,gateway:$("gateway").value,subnet:$("subnet").value,dns:$("dns").value,apFallback:$("apFallback").checked,apSsid:$("apSsid").value,apChannel:Number($("apChannel").value)};if($("password").value!=="")out.password=$("password").value;if($("apPassword").value!=="")out.apPassword=$("apPassword").value;if($("clearPassword").checked)out.clearPassword=true;if($("clearApPassword").checked)out.clearApPassword=true;return out}
...
async function scan(start=true){$("networks").textContent="Scanning";const d=await api(`/api/network/scan${start?"?start=1":""}`);if(d.scanning){setTimeout(()=>scan(false),1200);return}$("networks").innerHTML=`<h2>Networks</h2>${d.networks.map(n=>`<button data-ssid="${esc(n.ssid)}">${esc(n.ssid||"(hidden)")} (${n.rssi} dBm)</button>`).join("")||"No networks found"}`;document.querySelectorAll("[data-ssid]").forEach(b=>b.onclick=()=>{if(!b.dataset.ssid)return;$("ssid").value=b.dataset.ssid;$("hiddenSsid").checked=false;updateSummary()})}
document.querySelectorAll("input,select").forEach(el=>{el.oninput=updateSummary;el.onchange=updateSummary});$("password").oninput=()=>{if($("password").value)$("clearPassword").checked=false;updateSummary()};$("apPassword").oninput=()=>{if($("apPassword").value)$("clearApPassword").checked=false;updateSummary()};$("clearPassword").onchange=()=>{if($("clearPassword").checked)$("password").value="";updateSummary()};$("clearApPassword").onchange=()=>{if($("clearApPassword").checked)$("apPassword").value="";updateSummary()};document.querySelectorAll("[data-step]").forEach(b=>b.onclick=()=>{if(validate(step))showStep(Number(b.dataset.step))});$("back").onclick=()=>showStep(step-1);$("next").onclick=()=>{if(validate(step))showStep(step+1)};$("save").onclick=save;$("scan").onclick=()=>scan(true);load().then(()=>showStep(0)).catch(e=>$("status").textContent=e.message);
</script>
</body>
</html>
)HTML";
...
static void writeFloatValue(Print& out, float value) {
    // JSON has no NaN/Inf representation. Emit null instead of invalid JSON.
    if (!isfinite(value)) {
        out.print("null");
        return;
    }
...
static void populateSystemMetrics(JsonObject target) {
    // Object-builder version used by fallback JSON paths.
    SystemMetricsSnapshot metrics = systemMonitor.snapshot();

    JsonObject cpu = target["cpu"].to<JsonObject>();
    cpu["core0Percent"] = metrics.core0LoadPercent;
    cpu["waveformPercent"] = metrics.core1LoadPercent;

    JsonObject memory = target["memory"].to<JsonObject>();
    memory["heapTotalBytes"] = metrics.heapTotalBytes;
    memory["heapUsedBytes"] = metrics.heapUsedBytes;
    memory["heapFreeBytes"] = metrics.heapFreeBytes;
    memory["psramTotalBytes"] = metrics.psramTotalBytes;
    memory["psramUsedBytes"] = metrics.psramUsedBytes;
    memory["psramFreeBytes"] = metrics.psramFreeBytes;

    JsonObject flash = target["flash"].to<JsonObject>();
    flash["totalBytes"] = metrics.flashTotalBytes;
    flash["sketchUsedBytes"] = metrics.sketchUsedBytes;
    flash["sketchCapacityBytes"] = metrics.sketchCapacityBytes;
    flash["filesystemUsedBytes"] = metrics.filesystemUsedBytes;
...
static void streamSystemMetrics(Print& out, bool& first) {
    // Streaming version used by high-frequency status endpoints.
    SystemMetricsSnapshot metrics = systemMonitor.snapshot();

    beginObjectProp(out, first, "system");
    bool objectFirst = true;

    beginObjectProp(out, objectFirst, "cpu");
    bool nestedFirst = true;
    writeFloatProp(out, nestedFirst, "core0Percent", metrics.core0LoadPercent);
    writeFloatProp(out, nestedFirst, "waveformPercent", metrics.core1LoadPercent);
    out.write('}');

    beginObjectProp(out, objectFirst, "memory");
    nestedFirst = true;
    writeUIntProp(out, nestedFirst, "heapTotalBytes", metrics.heapTotalBytes);
    writeUIntProp(out, nestedFirst, "heapUsedBytes", metrics.heapUsedBytes);
    writeUIntProp(out, nestedFirst, "heapFreeBytes", metrics.heapFreeBytes);
    writeUIntProp(out, nestedFirst, "psramTotalBytes", metrics.psramTotalBytes);
    writeUIntProp(out, nestedFirst, "psramUsedBytes", metrics.psramUsedBytes);
    writeUIntProp(out, nestedFirst, "psramFreeBytes", metrics.psramFreeBytes);
    out.write('}');

    beginObjectProp(out, objectFirst, "flash");
    nestedFirst = true;
    writeUIntProp(out, nestedFirst, "totalBytes", metrics.flashTotalBytes);
    writeUIntProp(out, nestedFirst, "sketchUsedBytes", metrics.sketchUsedBytes);
    writeUIntProp(out, nestedFirst, "sketchCapacityBytes", metrics.sketchCapacityBytes);
    writeUIntProp(out, nestedFirst, "filesystemUsedBytes", metrics.filesystemUsedBytes);
...
static void streamOptions(Print& out) {
    ...
    beginArrayProp(out, firstOptionSet, "motorTopology");
    first = true;
    streamOptionPair(out, first, MOTOR_TOPOLOGY_CUSTOM, "Custom tuned phases");
    streamOptionPair(out, first, MOTOR_TOPOLOGY_TWIN_PHASE_SYNCHRONOUS, "Twin-phase synchronous");
    streamOptionPair(out, first, MOTOR_TOPOLOGY_THREE_PHASE, "Three-phase sine");
    out.write(']');
    ...
    beginArrayProp(out, firstOptionSet, "closedLoopControlMode");
    first = true;
    streamOptionPair(out, first, CLOSED_LOOP_CONTROL_MONITOR, "Monitor");
    streamOptionPair(out, first, CLOSED_LOOP_CONTROL_CORRECT, "Correct");
    out.write(']');

    beginArrayProp(out, firstOptionSet, "closedLoopSensorMode");
    first = true;
    streamOptionPair(out, first, CLOSED_LOOP_SENSOR_PULSE, "Pulse tach");
    streamOptionPair(out, first, CLOSED_LOOP_SENSOR_QUADRATURE, "Quadrature");
    out.write(']');

    beginArrayProp(out, firstOptionSet, "closedLoopPulseEdge");
    first = true;
    streamOptionPair(out, first, CLOSED_LOOP_EDGE_RISING, "Rising");
    streamOptionPair(out, first, CLOSED_LOOP_EDGE_FALLING, "Falling");
    streamOptionPair(out, first, CLOSED_LOOP_EDGE_CHANGE, "Change");
    out.write(']');

    beginArrayProp(out, firstOptionSet, "closedLoopQuadratureMode");
    first = true;
    streamOptionPair(out, first, CLOSED_LOOP_QUAD_X1, "x1");
    streamOptionPair(out, first, CLOSED_LOOP_QUAD_X2, "x2");
    streamOptionPair(out, first, CLOSED_LOOP_QUAD_X4, "x4");
    out.write(']');

    beginArrayProp(out, firstOptionSet, "closedLoopFaultAction");
    first = true;
    streamOptionPair(out, first, CLOSED_LOOP_FAULT_IGNORE, "Ignore");
    streamOptionPair(out, first, CLOSED_LOOP_FAULT_WARN, "Warn");
    streamOptionPair(out, first, CLOSED_LOOP_FAULT_STOP, "Stop");
    out.write(']');

    beginArrayProp(out, firstOptionSet, "closedLoopDropoutAction");
    first = true;
    streamOptionPair(out, first, CLOSED_LOOP_DROPOUT_OPEN_LOOP, "Open loop");
    streamOptionPair(out, first, CLOSED_LOOP_DROPOUT_HOLD, "Hold correction");
    streamOptionPair(out, first, CLOSED_LOOP_DROPOUT_STOP, "Stop");
    out.write(']');

    beginArrayProp(out, firstOptionSet, "closedLoopRampMode");
    first = true;
    streamOptionPair(out, first, CLOSED_LOOP_RAMP_DISABLED, "Off");
    ...
    streamOptionPair(out, first, CLOSED_LOOP_AMP_RECOVERY_WARN, "Warn");
    streamOptionPair(out, first, CLOSED_LOOP_AMP_RECOVERY_RESTORE, "Restore full amplitude");
    out.write(']');
#endif
    ...
    beginArrayProp(out, firstOptionSet, "standbyMode");
    first = true;
    streamOptionPair(out, first, NETWORK_STANDBY_NETWORK, "Network standby");
    streamOptionPair(out, first, NETWORK_STANDBY_ECO, "Eco standby");
    out.write(']');
...
static void streamGlobalGroups(Print& out) {
    ...
    beginFieldGroup(out, firstGroup, "Output Layout and Transitions");
    firstField = true;
    streamSelectField(out, firstField, "motorTopology", "Motor topology", "motorTopology", "Select nominal wiring and tuning context. Raw phase and gain controls remain editable in every topology.", true);
#if OUTPUT_STAGE_TYPE == OUTPUT_STAGE_3PWM_BRIDGE && POWER_STAGE_PHASE_ENABLES
    streamSelectField(out, firstField, "phaseMode", "Phase mode", "phaseMode", "Number of active bridge outputs. ENA, ENB, and ENC follow A, A+B, or A+B+C. Reductions apply immediately; newly added enables engage through the next motor-start sequence.", true);
#else
    streamSelectField(out, firstField, "phaseMode", "Phase mode", "phaseMode", "Number of active phase outputs.", true);
#endif
    streamNumberField(out, firstField, "phaseSlewDegreesPerSecond", "Phase slew", 0, 3600, 10, "Maximum rate for live phase-trim changes. Zero applies changes immediately.", "degrees/sec");
    streamNumberField(out, firstField, "gainSlewPercentPerSecond", "Gain slew", 0, 1000, 5, "Maximum rate for live per-channel gain changes. Zero applies changes immediately.", "percent/sec");
    endFieldGroup(out);

    beginFieldGroup(out, firstGroup, "Motor Amplitude");
    firstField = true;
#if OUTPUT_STAGE_TYPE == OUTPUT_STAGE_3PWM_BRIDGE
    streamNumberField(out, firstField, "maxAmplitude", "Maximum modulation", 0, 100, 1, "Upper bridge modulation-depth limit. Increase gradually while monitoring motor and driver current.", "percent", true);
#else
    streamNumberField(out, firstField, "maxAmplitude", "Maximum amplitude", 0, 100, 1, "Upper output amplitude limit.", "percent", true);
#endif
    streamNumberField(out, firstField, "vfBlend", "V/f blend", 0, 100, 1, "How strongly the V/f curve affects output amplitude.", "percent", true);
    streamNumberField(out, firstField, "vfLowFreq", "V/f low frequency", 0, 50, 1, "Low-frequency point on the V/f curve.", "Hz", true);
    streamNumberField(out, firstField, "vfLowLevel", "V/f low level", 0, 100, 1, "Output level at the low-frequency V/f point.", "percent", true);
    streamNumberField(out, firstField, "vfMidFreq", "V/f mid frequency", 0, 100, 1, "Mid-frequency point on the V/f curve.", "Hz", true);
    streamNumberField(out, firstField, "vfMidLevel", "V/f mid level", 0, 100, 1, "Output level at the mid-frequency V/f point.", "percent", true);
    streamNumberField(out, firstField, "vfBaseFreq", "V/f base frequency", MIN_OUTPUT_FREQUENCY_HZ, MAX_OUTPUT_FREQUENCY_HZ, 1, "Frequency at which the V/f curve reaches full output.", "Hz", true);
    endFieldGroup(out);

    beginFieldGroup(out, firstGroup, "Motor Ramping");
    firstField = true;
    ...
    beginFieldGroup(out, firstGroup, "Motor Braking");
    firstField = true;
#if OUTPUT_STAGE_TYPE == OUTPUT_STAGE_3PWM_BRIDGE
    streamCheckboxField(out, firstField, "activeBrakingAllowed", "Regenerative braking safe", "Confirm that the DC bus has a verified path for absorbing returned braking energy. This permits the selected active braking mode; it does not select one.", true);
#endif
...
#if CLOSED_LOOP_SPEED_ENABLE
    beginFieldGroup(out, firstGroup, "Closed Loop Control");
    firstField = true;
    streamCheckboxField(out, firstField, "closedLoopEnabled", "Closed-loop enabled", "Enable feedback correction after the motor reaches stable running state.", true);
    streamSelectField(out, firstField, "closedLoopControlMode", "Control mode", "closedLoopControlMode", "Monitor records RPM without changing frequency. Correct applies bounded feedback correction.", true);
    streamNumberField(out, firstField, "closedLoopTargetRpm33", "33 target RPM", 1, 120, 0.01f, "Feedback target for 33 RPM mode before pitch adjustment.", "RPM", true);
    streamNumberField(out, firstField, "closedLoopTargetRpm45", "45 target RPM", 1, 120, 0.01f, "Feedback target for 45 RPM mode before pitch adjustment.", "RPM", true);
    streamNumberField(out, firstField, "closedLoopTargetRpm78", "78 target RPM", 1, 120, 0.01f, "Feedback target for 78 RPM mode before pitch adjustment.", "RPM", true);
    streamNumberField(out, firstField, "closedLoopUpdateIntervalMs", "Update interval", 20, 1000, 10, "Controller update interval.", "ms", true);
    ...
    beginFieldGroup(out, firstGroup, "Closed Loop Sensor");
    firstField = true;
    streamSelectField(out, firstField, "closedLoopSensorMode", "Sensor mode", "closedLoopSensorMode", "Choose pulse tachometer or quadrature feedback input.", true);
    streamNumberField(out, firstField, "closedLoopCountsPerRev", "Counts per revolution", 1, 20000, 1, "Sensor counts per platter revolution after selected decode mode.", "count", true);
    streamSelectField(out, firstField, "closedLoopPulseEdge", "Pulse edge", "closedLoopPulseEdge", "Input edge counted in pulse tach mode.", true);
    streamSelectField(out, firstField, "closedLoopQuadratureMode", "Quadrature decode", "closedLoopQuadratureMode", "Decode density used for A/B quadrature inputs.", true);
    streamCheckboxField(out, firstField, "closedLoopReverseDirection", "Reverse direction", "Invert quadrature direction if the sensor runs backwards.", true);
    streamSelectField(out, firstField, "closedLoopDirectionFaultAction", "Direction fault action", "closedLoopFaultAction", "Action when quadrature feedback reports reverse motion.", true);
    streamNumberField(out, firstField, "closedLoopDebounceUs", "Debounce", 0, 50000, 10, "Minimum accepted time between sensor transitions.", "us", true);
    streamNumberField(out, firstField, "closedLoopTimeoutMs", "Signal timeout", 100, 10000, 100, "Maximum time without a valid count before feedback is considered lost.", "ms", true);
    endFieldGroup(out);

    beginFieldGroup(out, firstGroup, "Closed Loop Engage");
    firstField = true;
    streamNumberField(out, firstField, "closedLoopEngageDelayMs", "Engage delay", 0, 30000, 100, "Delay after stable running before feedback correction starts.", "ms", true);
    streamCheckboxField(out, firstField, "closedLoopRequireSignalBeforeEngage", "Require signal before engage", "Wait for a valid sensor signal before the controller can apply correction.", true);
    streamCheckboxField(out, firstField, "closedLoopRequireNearTargetBeforeEngage", "Require near target before engage", "Wait until measured speed is near the target before correction is allowed.", true);
    streamNumberField(out, firstField, "closedLoopEngageToleranceRpm", "Engage tolerance", 0.01f, 20, 0.01f, "Maximum RPM error allowed when requiring near-target engagement.", "RPM", true);
    endFieldGroup(out);

    beginFieldGroup(out, firstGroup, "Closed Loop Ramp");
    firstField = true;
    ...
    beginFieldGroup(out, firstGroup, "Closed Loop Pitch");
    firstField = true;
    ...
    beginFieldGroup(out, firstGroup, "Closed Loop Safety");
    firstField = true;
    ...
    streamSelectField(out, firstField, "closedLoopSaturationAction", "Saturation action", "closedLoopFaultAction", "Action after correction remains saturated for the configured time.", true);
    streamNumberField(out, firstField, "closedLoopPlausibilityMinRpm", "Minimum plausible RPM", 0, 120, 1, "Lowest measured RPM accepted as plausible. Zero disables the low check.", "RPM", true);
    streamNumberField(out, firstField, "closedLoopPlausibilityMaxRpm", "Maximum plausible RPM", 1, 200, 1, "Highest measured RPM accepted as plausible.", "RPM", true);
    streamSelectField(out, firstField, "closedLoopPlausibilityAction", "Plausibility action", "closedLoopFaultAction", "Action when measured RPM is outside the plausible range.", true);
    streamNumberField(out, firstField, "closedLoopLockTimeoutMs", "Lock timeout", 0, 60000, 100, "Time allowed after engagement to reach lock. Zero disables it.", "ms", true);
    streamSelectField(out, firstField, "closedLoopLockTimeoutAction", "Lock timeout action", "closedLoopFaultAction", "Action when feedback does not lock within the configured time.", true);
    streamSelectField(out, firstField, "closedLoopAmpRecoveryMode", "Amplitude recovery", "closedLoopAmpRecoveryMode", "Warn or restore full amplitude if reduced-amplitude mode cannot hold lock.", true);
    streamNumberField(out, firstField, "closedLoopAmpRecoveryDelayMs", "Amplitude recovery delay", 0, 30000, 100, "Out-of-lock time after amplitude reduction before recovery action runs.", "ms", true);
    endFieldGroup(out);
#endif

#if ENABLE_STANDBY || (OUTPUT_STAGE_TYPE == OUTPUT_STAGE_LINEAR_PWM && ENABLE_MUTE_RELAYS)
...
static void streamSpeedFields(Print& out) {
    ...
    streamNumberField(out, firstField, "gain0", "Channel 1 gain", 50, 150, 1, "Per-speed output balance trim. 100 percent is unity gain.", "percent", true);
    streamNumberField(out, firstField, "gain1", "Channel 2 gain", 50, 150, 1, "Per-speed output balance trim. 100 percent is unity gain.", "percent", true);
    streamNumberField(out, firstField, "gain2", "Channel 3 gain", 50, 150, 1, "Per-speed output balance trim. 100 percent is unity gain.", "percent", true);
    streamNumberField(out, firstField, "gain3", "Channel 4 gain", 50, 150, 1, "Per-speed output balance trim. 100 percent is unity gain.", "percent", true);
...
#if OUTPUT_STAGE_TYPE == OUTPUT_STAGE_3PWM_BRIDGE
    streamNumberField(out, firstField, "reducedAmplitude", "Reduced modulation", 10, 100, 1, "Running bridge modulation depth after the startup delay.", "percent", true);
#else
...
#if CLOSED_LOOP_SPEED_ENABLE
    streamNumberField(out, firstField, "closedLoopDeadbandRpm", "Closed-loop deadband", 0, 5, 0.01f, "RPM error band ignored by the controller for this speed.", "RPM", true);
    streamNumberField(out, firstField, "closedLoopLockToleranceRpm", "Closed-loop lock tolerance", 0.01f, 5, 0.01f, "RPM tolerance used to declare lock for this speed.", "RPM", true);
    streamNumberField(out, firstField, "closedLoopLockTimeMs", "Closed-loop lock time", 0, 30000, 100, "Time inside tolerance before lock is reported for this speed.", "ms", true);
    streamNumberField(out, firstField, "closedLoopKp", "Closed-loop Kp", 0, 20, 0.01f, "Proportional correction gain for this speed.", nullptr, true);
    streamNumberField(out, firstField, "closedLoopKi", "Closed-loop Ki", 0, 20, 0.01f, "Integral correction gain for this speed.", nullptr, true);
    streamNumberField(out, firstField, "closedLoopKd", "Closed-loop Kd", 0, 20, 0.01f, "Derivative correction gain for this speed.", nullptr, true);
    streamNumberField(out, firstField, "closedLoopIntegralLimitHz", "Closed-loop integral limit", 0, 50, 0.1f, "Maximum integral contribution for this speed.", "Hz", true);
    streamNumberField(out, firstField, "closedLoopCorrectionLimitHz", "Closed-loop correction limit", 0, 100, 0.1f, "Maximum feedback correction for this speed.", "Hz", true);
    streamNumberField(out, firstField, "closedLoopSlewLimitHzPerSec", "Closed-loop slew limit", 0, 100, 0.1f, "Maximum correction change rate for this speed.", "Hz/s", true);
    streamNumberField(out, firstField, "closedLoopRampKp", "Closed-loop ramp Kp", 0, 5, 0.01f, "Ramp-tracking proportional gain for this speed.", nullptr, true);
    streamNumberField(out, firstField, "closedLoopRampCorrectionLimitHz", "Closed-loop ramp limit", 0, 20, 0.1f, "Maximum ramp-tracking correction for this speed.", "Hz", true);
#endif
}
...
static void streamNetworkFields(Print& out) {
    ...
    streamSelectField(out, firstField, "standbyMode", "Standby mode", "standbyMode", "Network standby keeps Wi-Fi active while the controller is in standby. Eco standby turns Wi-Fi off during standby and reconnects on physical wake.");
...
static void streamSchema(Print& out) {
    // Stream the schema directly rather than constructing a large JsonDocument.
    out.write('{');
    bool first = true;
...
static bool presetSlotExists(int slot) {
    // Preset file names use zero-based slot indexes.
    char path[24];
...
void WebInterface::update() {
    // Keep this non-blocking; it runs from Core 0 alongside motor/UI work.
...
void WebInterface::handleSchemaGet() {
    ...
    // Measure first so the client receives a normal Content-Length response.
...
void WebInterface::populateStatus(JsonDocument& doc) {
    // Fallback status builder used when chunked streaming is unavailable.
    ...
    motorJson["outputBackend"] = powerStage.backendName();
    motorJson["driverEnabled"] = powerStage.isEnabled();
    motorJson["driverEnablePending"] = powerStage.isEnablePending();
    motorJson["driverFault"] = powerStage.hasFault();
    JsonObject outputJson = motorJson["output"].to<JsonObject>();
    outputJson["state"] = powerStage.stateName();
    outputJson["stateCode"] = (int)powerStage.state();
    PowerStageMetrics stageMetrics = powerStage.metrics();
    JsonObject stageMetricsJson = outputJson["metrics"].to<JsonObject>();
    stageMetricsJson["enableAttempts"] = stageMetrics.enableAttempts;
    stageMetricsJson["successfulEnables"] = stageMetrics.successfulEnables;
    stageMetricsJson["faultCount"] = stageMetrics.faultCount;
    stageMetricsJson["wakeFaultCount"] = stageMetrics.wakeFaultCount;
    stageMetricsJson["runningFaultCount"] = stageMetrics.runningFaultCount;
    JsonArray channelsJson = outputJson["channels"].to<JsonArray>();
    SpeedSettings& outputSpeed = settings.getCurrentSpeedSettings();
    for (uint8_t channel = 0; channel < 4; channel++) {
        JsonObject channelJson = channelsJson.add<JsonObject>();
        channelJson["targetPhase"] = outputSpeed.phaseOffset[channel];
        channelJson["appliedPhase"] = waveform.getAppliedPhaseDegrees(channel);
        channelJson["targetGain"] = outputSpeed.channelAmplitude[channel];
        channelJson["appliedGain"] = waveform.getAppliedChannelGainPercent(channel);
        channelJson["headroom"] = waveform.getModulationHeadroomPercent(channel);
        channelJson["clippingCount"] = waveform.getClippingCount(channel);
    }
    PowerStageFaultSnapshot faultSnapshot = powerStage.faultSnapshot();
    JsonObject faultJson = outputJson["lastFault"].to<JsonObject>();
    faultJson["valid"] = faultSnapshot.valid;
    if (faultSnapshot.valid) {
        faultJson["timestampMs"] = faultSnapshot.timestampMs;
        faultJson["originState"] = faultSnapshot.originState;
        faultJson["motorState"] = faultSnapshot.motorState;
        faultJson["speed"] = faultSnapshot.speed;
        faultJson["frequencyHz"] = faultSnapshot.frequencyHz;
        faultJson["bufferFillCount"] = faultSnapshot.bufferFillCount;
        JsonArray faultClips = faultJson["clippingCount"].to<JsonArray>();
        JsonArray faultPhases = faultJson["phaseOffset"].to<JsonArray>();
        JsonArray faultGains = faultJson["channelGain"].to<JsonArray>();
        for (uint8_t channel = 0; channel < 4; channel++) {
            faultClips.add(faultSnapshot.clippingCount[channel]);
            faultPhases.add(faultSnapshot.phaseOffset[channel]);
            faultGains.add(faultSnapshot.channelGain[channel]);
        }
    }
    ...
    closedLoop["compiled"] = true;
    closedLoop["enabled"] = settings.get().closedLoopEnabled;
    closedLoop["controlMode"] = settings.get().closedLoopControlMode;
    closedLoop["sensorMode"] = settings.get().closedLoopSensorMode;
    closedLoop["active"] = motor.isClosedLoopActive();
    closedLoop["saturated"] = motor.isClosedLoopSaturated();
    closedLoop["ampRecoveryActive"] = motor.isClosedLoopAmpRecoveryActive();
    ...
    closedLoop["correctionHz"] = motor.getClosedLoopCorrectionHz();
    closedLoop["signalValid"] = feedback.signalValid;
    closedLoop["locked"] = feedback.locked;
    closedLoop["direction"] = feedback.direction;
    closedLoop["rawDirection"] = feedback.rawDirection;
    closedLoop["pinAHigh"] = feedback.pinAHigh;
    closedLoop["pinBHigh"] = feedback.pinBHigh;
    closedLoop["count"] = feedback.count;
    closedLoop["countDelta"] = feedback.countDelta;
    ...
    setupJson["pinBHigh"] = setup.pinBHigh;
    setupJson["elapsedMs"] = setup.elapsedMs;
    setupJson["countDelta"] = setup.countDelta;
    setupJson["invalidDelta"] = setup.invalidDelta;
    setupJson["debouncedDelta"] = setup.debouncedDelta;
    setupJson["rawDirection"] = setup.rawDirection;
    setupJson["correctedDirection"] = setup.correctedDirection;
    setupJson["suggestedCountsPerRev"] = setup.suggestedCountsPerRev;
    setupJson["suggestedReverseDirection"] = setup.suggestedReverseDirection;
    ClosedLoopTuningStatus tuning = motor.getClosedLoopTuningStatus();
    ClosedLoopMetrics metrics = tuning.metrics;
    JsonObject metricsJson = closedLoop["metrics"].to<JsonObject>();
    metricsJson["sampleCount"] = metrics.sampleCount;
    metricsJson["validSamples"] = metrics.validSamples;
    metricsJson["lockedSamples"] = metrics.lockedSamples;
    metricsJson["validMs"] = metrics.validMs;
    metricsJson["lockedMs"] = metrics.lockedMs;
    metricsJson["saturatedMs"] = metrics.saturatedMs;
    metricsJson["dropoutEvents"] = metrics.dropoutEvents;
    metricsJson["saturationEvents"] = metrics.saturationEvents;
    metricsJson["directionFaultEvents"] = metrics.directionFaultEvents;
    metricsJson["plausibilityEvents"] = metrics.plausibilityEvents;
    metricsJson["lockTimeoutEvents"] = metrics.lockTimeoutEvents;
    metricsJson["ampRecoveryEvents"] = metrics.ampRecoveryEvents;
    metricsJson["errorSignChanges"] = metrics.errorSignChanges;
    metricsJson["averageErrorRpm"] = metrics.averageErrorRpm;
    ...
    net["standbyMode"] = cfg.standbyMode;
    net["ecoStandbySuspended"] = networkManager.isEcoStandbySuspended();
...
void WebInterface::streamStatus(Print& out) {
    ...
    writeStringProp(out, objectFirst, "outputBackend", powerStage.backendName());
    writeBoolProp(out, objectFirst, "driverEnabled", powerStage.isEnabled());
    writeBoolProp(out, objectFirst, "driverEnablePending", powerStage.isEnablePending());
    writeBoolProp(out, objectFirst, "driverFault", powerStage.hasFault());
    beginObjectProp(out, objectFirst, "output");
    bool outputFirst = true;
    writeStringProp(out, outputFirst, "state", powerStage.stateName());
    writeIntProp(out, outputFirst, "stateCode", (long)powerStage.state());
    PowerStageMetrics stageMetrics = powerStage.metrics();
    beginObjectProp(out, outputFirst, "metrics");
    bool stageMetricsFirst = true;
    writeUIntProp(out, stageMetricsFirst, "enableAttempts", stageMetrics.enableAttempts);
    writeUIntProp(out, stageMetricsFirst, "successfulEnables", stageMetrics.successfulEnables);
    writeUIntProp(out, stageMetricsFirst, "faultCount", stageMetrics.faultCount);
    writeUIntProp(out, stageMetricsFirst, "wakeFaultCount", stageMetrics.wakeFaultCount);
    writeUIntProp(out, stageMetricsFirst, "runningFaultCount", stageMetrics.runningFaultCount);
    out.write('}');
    beginArrayProp(out, outputFirst, "channels");
    bool channelFirst = true;
    SpeedSettings& outputSpeed = settings.getCurrentSpeedSettings();
    for (uint8_t channel = 0; channel < 4; channel++) {
        writeComma(out, channelFirst);
        out.write('{');
        bool valueFirst = true;
        writeFloatProp(out, valueFirst, "targetPhase", outputSpeed.phaseOffset[channel]);
        writeFloatProp(out, valueFirst, "appliedPhase", waveform.getAppliedPhaseDegrees(channel));
        writeIntProp(out, valueFirst, "targetGain", outputSpeed.channelAmplitude[channel]);
        writeFloatProp(out, valueFirst, "appliedGain", waveform.getAppliedChannelGainPercent(channel));
        writeFloatProp(out, valueFirst, "headroom", waveform.getModulationHeadroomPercent(channel));
        writeUIntProp(out, valueFirst, "clippingCount", waveform.getClippingCount(channel));
        out.write('}');
    }
    out.write(']');
    PowerStageFaultSnapshot faultSnapshot = powerStage.faultSnapshot();
    beginObjectProp(out, outputFirst, "lastFault");
    bool faultFirst = true;
    writeBoolProp(out, faultFirst, "valid", faultSnapshot.valid);
    if (faultSnapshot.valid) {
        writeUIntProp(out, faultFirst, "timestampMs", faultSnapshot.timestampMs);
        writeIntProp(out, faultFirst, "originState", faultSnapshot.originState);
        writeIntProp(out, faultFirst, "motorState", faultSnapshot.motorState);
        writeIntProp(out, faultFirst, "speed", faultSnapshot.speed);
        writeFloatProp(out, faultFirst, "frequencyHz", faultSnapshot.frequencyHz);
        writeUIntProp(out, faultFirst, "bufferFillCount", faultSnapshot.bufferFillCount);
        beginArrayProp(out, faultFirst, "clippingCount");
        for (uint8_t channel = 0; channel < 4; channel++) { if (channel) out.write(','); out.print(faultSnapshot.clippingCount[channel]); }
        out.write(']');
        beginArrayProp(out, faultFirst, "phaseOffset");
        for (uint8_t channel = 0; channel < 4; channel++) { if (channel) out.write(','); out.print(faultSnapshot.phaseOffset[channel], 3); }
        out.write(']');
        beginArrayProp(out, faultFirst, "channelGain");
        for (uint8_t channel = 0; channel < 4; channel++) { if (channel) out.write(','); out.print(faultSnapshot.channelGain[channel]); }
        out.write(']');
    }
    out.write('}');
    out.write('}');
    ...
    bool nestedFirst = true;
    writeBoolProp(out, nestedFirst, "compiled", true);
    writeBoolProp(out, nestedFirst, "enabled", settings.get().closedLoopEnabled);
    writeIntProp(out, nestedFirst, "controlMode", settings.get().closedLoopControlMode);
    writeIntProp(out, nestedFirst, "sensorMode", settings.get().closedLoopSensorMode);
    writeBoolProp(out, nestedFirst, "active", motor.isClosedLoopActive());
    writeBoolProp(out, nestedFirst, "saturated", motor.isClosedLoopSaturated());
    writeBoolProp(out, nestedFirst, "ampRecoveryActive", motor.isClosedLoopAmpRecoveryActive());
    ...
    writeFloatProp(out, nestedFirst, "correctionHz", motor.getClosedLoopCorrectionHz());
    writeBoolProp(out, nestedFirst, "signalValid", feedback.signalValid);
    writeBoolProp(out, nestedFirst, "locked", feedback.locked);
    writeIntProp(out, nestedFirst, "direction", feedback.direction);
    writeIntProp(out, nestedFirst, "rawDirection", feedback.rawDirection);
    writeBoolProp(out, nestedFirst, "pinAHigh", feedback.pinAHigh);
    writeBoolProp(out, nestedFirst, "pinBHigh", feedback.pinBHigh);
    writeIntProp(out, nestedFirst, "count", feedback.count);
    writeIntProp(out, nestedFirst, "countDelta", feedback.countDelta);
    ...
    writeBoolProp(out, setupFirst, "pinAHigh", setup.pinAHigh);
    writeBoolProp(out, setupFirst, "pinBHigh", setup.pinBHigh);
    writeUIntProp(out, setupFirst, "elapsedMs", setup.elapsedMs);
    writeIntProp(out, setupFirst, "countDelta", setup.countDelta);
    writeUIntProp(out, setupFirst, "invalidDelta", setup.invalidDelta);
    writeUIntProp(out, setupFirst, "debouncedDelta", setup.debouncedDelta);
    writeIntProp(out, setupFirst, "rawDirection", setup.rawDirection);
    writeIntProp(out, setupFirst, "correctedDirection", setup.correctedDirection);
    ...
    beginObjectProp(out, objectFirst, "closedLoop");
    bool nestedFirst = true;
    writeBoolProp(out, nestedFirst, "compiled", false);
    out.write('}');
#endif
    out.write('}');
    ...
    writeIntProp(out, objectFirst, "standbyMode", cfg.standbyMode);
    writeBoolProp(out, objectFirst, "ecoStandbySuspended", networkManager.isEcoStandbySuspended());
    out.write('}');
...
void WebInterface::handleSettingsGet() {
    ...
    global["motorTopology"] = g.motorTopology;
#if OUTPUT_STAGE_TYPE == OUTPUT_STAGE_3PWM_BRIDGE
    global["activeBrakingAllowed"] = g.activeBrakingAllowed;
#endif
    global["phaseSlewDegreesPerSecond"] = g.phaseSlewDegreesPerSecond;
    global["gainSlewPercentPerSecond"] = g.gainSlewPercentPerSecond;
    ...
    global["showCpuDashboard"] = g.showCpuDashboard;
    global["showMemoryDashboard"] = g.showMemoryDashboard;
    global["showFlashDashboard"] = g.showFlashDashboard;
    ...
    global["vfBlend"] = g.vfBlend;
    global["vfLowFreq"] = g.vfLowFreq;
    global["vfLowLevel"] = g.vfLowLevel;
    global["vfMidFreq"] = g.vfMidFreq;
    global["vfMidLevel"] = g.vfMidLevel;
    global["vfBaseFreq"] = g.vfBaseFreq;
#if CLOSED_LOOP_SPEED_ENABLE
    global["closedLoopEnabled"] = g.closedLoopEnabled;
    global["closedLoopControlMode"] = g.closedLoopControlMode;
    global["closedLoopSensorMode"] = g.closedLoopSensorMode;
    global["closedLoopTargetRpm33"] = g.closedLoopTargetRpm[SPEED_33];
    global["closedLoopTargetRpm45"] = g.closedLoopTargetRpm[SPEED_45];
    global["closedLoopTargetRpm78"] = g.closedLoopTargetRpm[SPEED_78];
    global["closedLoopCountsPerRev"] = g.closedLoopCountsPerRev;
    global["closedLoopPulseEdge"] = g.closedLoopPulseEdge;
    global["closedLoopQuadratureMode"] = g.closedLoopQuadratureMode;
    global["closedLoopReverseDirection"] = g.closedLoopReverseDirection;
    global["closedLoopDirectionFaultAction"] = g.closedLoopDirectionFaultAction;
    global["closedLoopDebounceUs"] = g.closedLoopDebounceUs;
    global["closedLoopTimeoutMs"] = g.closedLoopTimeoutMs;
    global["closedLoopEngageDelayMs"] = g.closedLoopEngageDelayMs;
    global["closedLoopUpdateIntervalMs"] = g.closedLoopUpdateIntervalMs;
    global["closedLoopFilterAlpha"] = g.closedLoopFilterAlpha;
    global["closedLoopDropoutAction"] = g.closedLoopDropoutAction;
    global["closedLoopRequireSignalBeforeEngage"] = g.closedLoopRequireSignalBeforeEngage;
    global["closedLoopRequireNearTargetBeforeEngage"] = g.closedLoopRequireNearTargetBeforeEngage;
    global["closedLoopEngageToleranceRpm"] = g.closedLoopEngageToleranceRpm;
    ...
    global["closedLoopPlausibilityMaxRpm"] = g.closedLoopPlausibilityMaxRpm;
    global["closedLoopPlausibilityAction"] = g.closedLoopPlausibilityAction;
    global["closedLoopLockTimeoutMs"] = g.closedLoopLockTimeoutMs;
    global["closedLoopLockTimeoutAction"] = g.closedLoopLockTimeoutAction;
    global["closedLoopAmpRecoveryMode"] = g.closedLoopAmpRecoveryMode;
    global["closedLoopAmpRecoveryDelayMs"] = g.closedLoopAmpRecoveryDelayMs;
#endif
    ...
    for (int i = 0; i < 3; i++) {
        ...
        JsonArray gains = speed["channelAmplitude"].to<JsonArray>();
        for (int p = 0; p < 4; p++) gains.add(s.channelAmplitude[p]);
        ...
        speed["closedLoopDeadbandRpm"] = t.deadbandRpm;
        speed["closedLoopLockToleranceRpm"] = t.lockToleranceRpm;
        speed["closedLoopLockTimeMs"] = t.lockTimeMs;
        speed["closedLoopKp"] = t.kp;
        speed["closedLoopKi"] = t.ki;
        speed["closedLoopKd"] = t.kd;
        speed["closedLoopIntegralLimitHz"] = t.integralLimitHz;
        speed["closedLoopCorrectionLimitHz"] = t.correctionLimitHz;
        speed["closedLoopSlewLimitHzPerSec"] = t.slewLimitHzPerSec;
        speed["closedLoopRampKp"] = t.rampKp;
        speed["closedLoopRampCorrectionLimitHz"] = t.rampCorrectionLimitHz;
#endif
    }
...
void WebInterface::handleSettingsPost() {
    ...
    if (!global.isNull()) {
        ...
        setByte(global, "motorTopology", g.motorTopology, MOTOR_TOPOLOGY_CUSTOM, MOTOR_TOPOLOGY_THREE_PHASE);
#if OUTPUT_STAGE_TYPE == OUTPUT_STAGE_3PWM_BRIDGE
        setBool(global, "activeBrakingAllowed", g.activeBrakingAllowed);
#endif
        setFloat(global, "phaseSlewDegreesPerSecond", g.phaseSlewDegreesPerSecond, 0.0f, 3600.0f);
        setFloat(global, "gainSlewPercentPerSecond", g.gainSlewPercentPerSecond, 0.0f, 1000.0f);
        ...
        setBool(global, "showCpuDashboard", g.showCpuDashboard);
        setBool(global, "showMemoryDashboard", g.showMemoryDashboard);
        setBool(global, "showFlashDashboard", g.showFlashDashboard);
        ...
        setByte(global, "vfBlend", g.vfBlend, 0, 100);
        setFloat(global, "vfLowFreq", g.vfLowFreq, 0.0f, 50.0f);
        setByte(global, "vfLowLevel", g.vfLowLevel, 0, 100);
        setFloat(global, "vfMidFreq", g.vfMidFreq, 0.0f, 100.0f);
        setByte(global, "vfMidLevel", g.vfMidLevel, 0, 100);
        setFloat(global, "vfBaseFreq", g.vfBaseFreq, MIN_OUTPUT_FREQUENCY_HZ, MAX_OUTPUT_FREQUENCY_HZ);
#if CLOSED_LOOP_SPEED_ENABLE
        setBool(global, "closedLoopEnabled", g.closedLoopEnabled);
        setByte(global, "closedLoopControlMode", g.closedLoopControlMode, CLOSED_LOOP_CONTROL_MONITOR, CLOSED_LOOP_CONTROL_CORRECT);
        setByte(global, "closedLoopSensorMode", g.closedLoopSensorMode, CLOSED_LOOP_SENSOR_PULSE, CLOSED_LOOP_SENSOR_QUADRATURE);
        setFloat(global, "closedLoopTargetRpm33", g.closedLoopTargetRpm[SPEED_33], 1.0f, 120.0f);
        setFloat(global, "closedLoopTargetRpm45", g.closedLoopTargetRpm[SPEED_45], 1.0f, 120.0f);
        setFloat(global, "closedLoopTargetRpm78", g.closedLoopTargetRpm[SPEED_78], 1.0f, 120.0f);
        setUInt16(global, "closedLoopCountsPerRev", g.closedLoopCountsPerRev, 1, 20000);
        setByte(global, "closedLoopPulseEdge", g.closedLoopPulseEdge, CLOSED_LOOP_EDGE_RISING, CLOSED_LOOP_EDGE_CHANGE);
        setByte(global, "closedLoopQuadratureMode", g.closedLoopQuadratureMode, CLOSED_LOOP_QUAD_X1, CLOSED_LOOP_QUAD_X4);
        setBool(global, "closedLoopReverseDirection", g.closedLoopReverseDirection);
        setByte(global, "closedLoopDirectionFaultAction", g.closedLoopDirectionFaultAction, CLOSED_LOOP_FAULT_IGNORE, CLOSED_LOOP_FAULT_STOP);
        setUInt16(global, "closedLoopDebounceUs", g.closedLoopDebounceUs, 0, 50000);
        setUInt16(global, "closedLoopTimeoutMs", g.closedLoopTimeoutMs, 100, 10000);
        setUInt16(global, "closedLoopEngageDelayMs", g.closedLoopEngageDelayMs, 0, 30000);
        setUInt16(global, "closedLoopUpdateIntervalMs", g.closedLoopUpdateIntervalMs, 20, 1000);
        setFloat(global, "closedLoopFilterAlpha", g.closedLoopFilterAlpha, 0.01f, 1.0f);
        setFloat(global, "closedLoopDeadbandRpm", g.closedLoopDeadbandRpm, 0.0f, 5.0f);
        setFloat(global, "closedLoopLockToleranceRpm", g.closedLoopLockToleranceRpm, 0.01f, 5.0f);
        setUInt16(global, "closedLoopLockTimeMs", g.closedLoopLockTimeMs, 0, 30000);
        setFloat(global, "closedLoopKp", g.closedLoopKp, 0.0f, 20.0f);
        setFloat(global, "closedLoopKi", g.closedLoopKi, 0.0f, 20.0f);
        setFloat(global, "closedLoopKd", g.closedLoopKd, 0.0f, 20.0f);
        setFloat(global, "closedLoopIntegralLimitHz", g.closedLoopIntegralLimitHz, 0.0f, 50.0f);
        setFloat(global, "closedLoopCorrectionLimitHz", g.closedLoopCorrectionLimitHz, 0.0f, 100.0f);
        setFloat(global, "closedLoopSlewLimitHzPerSec", g.closedLoopSlewLimitHzPerSec, 0.0f, 100.0f);
        setByte(global, "closedLoopDropoutAction", g.closedLoopDropoutAction, CLOSED_LOOP_DROPOUT_OPEN_LOOP, CLOSED_LOOP_DROPOUT_STOP);
        setBool(global, "closedLoopRequireSignalBeforeEngage", g.closedLoopRequireSignalBeforeEngage);
        setBool(global, "closedLoopRequireNearTargetBeforeEngage", g.closedLoopRequireNearTargetBeforeEngage);
        setFloat(global, "closedLoopEngageToleranceRpm", g.closedLoopEngageToleranceRpm, 0.01f, 20.0f);
        setByte(global, "closedLoopRampMode", g.closedLoopRampMode, CLOSED_LOOP_RAMP_DISABLED, CLOSED_LOOP_RAMP_TRACK);
        setFloat(global, "closedLoopRampKp", g.closedLoopRampKp, 0.0f, 5.0f);
        setFloat(global, "closedLoopRampCorrectionLimitHz", g.closedLoopRampCorrectionLimitHz, 0.0f, 20.0f);
        for (uint8_t i = 0; i < 3; i++) {
            ClosedLoopSpeedTuning& t = g.closedLoopTuning[i];
            if (!global["closedLoopDeadbandRpm"].isNull()) t.deadbandRpm = g.closedLoopDeadbandRpm;
            if (!global["closedLoopLockToleranceRpm"].isNull()) t.lockToleranceRpm = g.closedLoopLockToleranceRpm;
            if (!global["closedLoopLockTimeMs"].isNull()) t.lockTimeMs = g.closedLoopLockTimeMs;
            if (!global["closedLoopKp"].isNull()) t.kp = g.closedLoopKp;
            if (!global["closedLoopKi"].isNull()) t.ki = g.closedLoopKi;
            if (!global["closedLoopKd"].isNull()) t.kd = g.closedLoopKd;
            if (!global["closedLoopIntegralLimitHz"].isNull()) t.integralLimitHz = g.closedLoopIntegralLimitHz;
            if (!global["closedLoopCorrectionLimitHz"].isNull()) t.correctionLimitHz = g.closedLoopCorrectionLimitHz;
            if (!global["closedLoopSlewLimitHzPerSec"].isNull()) t.slewLimitHzPerSec = g.closedLoopSlewLimitHzPerSec;
            if (!global["closedLoopRampKp"].isNull()) t.rampKp = g.closedLoopRampKp;
            if (!global["closedLoopRampCorrectionLimitHz"].isNull()) t.rampCorrectionLimitHz = g.closedLoopRampCorrectionLimitHz;
        }
        ...
        setFloat(global, "closedLoopPlausibilityMaxRpm", g.closedLoopPlausibilityMaxRpm, 1.0f, 200.0f);
        setByte(global, "closedLoopPlausibilityAction", g.closedLoopPlausibilityAction, CLOSED_LOOP_FAULT_IGNORE, CLOSED_LOOP_FAULT_STOP);
        setUInt16(global, "closedLoopLockTimeoutMs", g.closedLoopLockTimeoutMs, 0, 60000);
        setByte(global, "closedLoopLockTimeoutAction", g.closedLoopLockTimeoutAction, CLOSED_LOOP_FAULT_IGNORE, CLOSED_LOOP_FAULT_STOP);
        setByte(global, "closedLoopAmpRecoveryMode", g.closedLoopAmpRecoveryMode, CLOSED_LOOP_AMP_RECOVERY_OFF, CLOSED_LOOP_AMP_RECOVERY_RESTORE);
        setUInt16(global, "closedLoopAmpRecoveryDelayMs", g.closedLoopAmpRecoveryDelayMs, 0, 30000);
#endif
    ...
    if (!speeds.isNull()) {
        for (size_t i = 0; i < 3 && i < speeds.size(); i++) {
            ...
            JsonArray gains = speed["channelAmplitude"].as<JsonArray>();
            if (!speed["channelAmplitude"].isNull() && !speed["channelAmplitude"].is<JsonArray>()) {
                rejectJsonValue("channelAmplitude", "an array");
            }
            if (!gains.isNull()) {
                for (size_t p = 0; p < 4 && p < gains.size(); p++) {
                    if (!gains[p].is<uint8_t>()) {
                        rejectJsonValue("channelAmplitude value", "an integer");
                    } else {
                        int gain = gains[p].as<int>();
                        if (gain < 50) gain = 50;
                        if (gain > 150) gain = 150;
                        s.channelAmplitude[p] = (uint8_t)gain;
                    }
                }
            }
...
#if CLOSED_LOOP_SPEED_ENABLE
            ClosedLoopSpeedTuning& t = g.closedLoopTuning[i];
            setFloat(speed, "closedLoopDeadbandRpm", t.deadbandRpm, 0.0f, 5.0f);
            setFloat(speed, "closedLoopLockToleranceRpm", t.lockToleranceRpm, 0.01f, 5.0f);
            setUInt16(speed, "closedLoopLockTimeMs", t.lockTimeMs, 0, 30000);
            setFloat(speed, "closedLoopKp", t.kp, 0.0f, 20.0f);
            setFloat(speed, "closedLoopKi", t.ki, 0.0f, 20.0f);
            setFloat(speed, "closedLoopKd", t.kd, 0.0f, 20.0f);
            setFloat(speed, "closedLoopIntegralLimitHz", t.integralLimitHz, 0.0f, 50.0f);
            setFloat(speed, "closedLoopCorrectionLimitHz", t.correctionLimitHz, 0.0f, 100.0f);
            setFloat(speed, "closedLoopSlewLimitHzPerSec", t.slewLimitHzPerSec, 0.0f, 100.0f);
            setFloat(speed, "closedLoopRampKp", t.rampKp, 0.0f, 5.0f);
            setFloat(speed, "closedLoopRampCorrectionLimitHz", t.rampCorrectionLimitHz, 0.0f, 20.0f);
#endif
        }
    }
...
void WebInterface::handleControl() {
    ...
    if (strcmp(action, "start") == 0) {
    ...
    } else if (strcmp(action, "closedLoopSetupStart") == 0) {
        speedFeedback.beginSetupCapture();
    } else if (strcmp(action, "closedLoopSetupApply") == 0) {
        SpeedFeedbackSetupStatus setup = speedFeedback.getSetupStatus();
        if (!setup.active || setup.suggestedCountsPerRev == 0) {
            sendError(409, "No closed-loop setup capture is ready to apply");
            return;
        }
        ...
        settings.normalize();
        motor.applySettings();
    } else if (strcmp(action, "closedLoopSetupStop") == 0) {
        speedFeedback.cancelSetupCapture();
    } else if (strcmp(action, "closedLoopTuneStart") == 0) {
        ...
        uint8_t relayStageCount = motor.getRelayTestStageCount();
        if (relayStageCount == 0) {
            sendError(409, "Relay test is not available in this output configuration");
            return;
        }
        if (!doc["stage"].is<int>()) {
        ...
        uint8_t stage = (uint8_t)clampIntValue(doc["stage"].as<int>(), 0, relayStageCount - 1);
        ...
        if (motor.getRelayTestStageCount() == 0) {
            sendError(409, "Relay test is not available in this output configuration");
            return;
        }
        motor.endRelayTest();
...
void WebInterface::handleNetworkGet() {
    ...
    doc["standbyModeText"] = networkStandbyModeName(c.standbyMode);
    doc["ecoStandbySuspended"] = networkManager.isEcoStandbySuspended();
    doc["ip"] = ip;
    ...
    config["ssid"] = c.ssid;
    config["hiddenSsid"] = c.hiddenSsid;
    config["password"] = "";
    ...
    config["standbyMode"] = c.standbyMode;
...
void WebInterface::handleNetworkPost() {
    ...
    setBool(doc.as<JsonObject>(), "hiddenSsid", c.hiddenSsid);
    setBool(doc.as<JsonObject>(), "apFallback", c.apFallback);
    setByte(doc.as<JsonObject>(), "standbyMode", c.standbyMode, NETWORK_STANDBY_NETWORK, NETWORK_STANDBY_ECO);
...
void WebInterface::handleDiagnosticsGet() {
    ...
    flags["CLOSED_LOOP_SPEED_ENABLE"] = CLOSED_LOOP_SPEED_ENABLE;
    ...
    flags["OUTPUT_STAGE_TYPE"] = OUTPUT_STAGE_TYPE;
    flags["POWER_STAGE_FAULT_ENABLE"] = POWER_STAGE_FAULT_ENABLE;
    flags["POWER_STAGE_SHARED_ENABLE"] = POWER_STAGE_SHARED_ENABLE;
    flags["POWER_STAGE_PHASE_ENABLES"] = POWER_STAGE_PHASE_ENABLES;
    ...
    pins["Driver enable/fault"] = PIN_POWER_STAGE_ENABLE;
#else
    pins["Driver fault"] = PIN_POWER_STAGE_FAULT;
#endif
#endif
#if POWER_STAGE_PHASE_ENABLES
    pins["Driver enable A"] = PIN_POWER_STAGE_PHASE_ENABLE_A;
    pins["Driver enable B"] = PIN_POWER_STAGE_PHASE_ENABLE_B;
    pins["Driver enable C"] = PIN_POWER_STAGE_PHASE_ENABLE_C;
#endif
#if POWER_STAGE_SLEEP_ENABLE
    pins["Driver sleep"] = PIN_POWER_STAGE_SLEEP;
#endif
#if POWER_STAGE_RESET_ENABLE
    pins["Driver reset"] = PIN_POWER_STAGE_RESET;
#endif
#endif
#if CLOSED_LOOP_SPEED_ENABLE
    ...
    network["ecoStandbySuspended"] = networkManager.isEcoStandbySuspended();
...
void WebInterface::handlePresetPost() {
    ...
    JsonDocument doc;
    if (!parseBody(doc)) return;
    JsonVariant slotValue = doc["slot"];
    if (slotValue.isNull()) {
        sendError(400, "Missing preset slot");
        return;
    }
    int slot = slotValue.as<int>();
    if (slot < 0 || slot >= MAX_PRESET_SLOTS) {
        sendError(400, "Preset slot must be 0 through 4");
        return;
    }
    const char* action = doc["action"].as<const char*>();
    if (!action) {
